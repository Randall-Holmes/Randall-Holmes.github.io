...
The well-ordering $\leq_\alpha^+$ of $\tau_\alpha^+$ must satisfy the condition that for each $(x,S) \in \tau_\alpha^+$, for each $(z,A) \in {\tt rng}(S)$, $\iota_*^+(y,T) <_\alpha^+ \iota_*^+(x,S)$ must hold [where $\iota_*^+$ computes ordinal position in any $\leq^+_\beta$] if $f_{\beta,\gamma}(y,T) = L$  with $\beta<\alpha$, $z \in \tau_\gamma$ and
...
This hypothesis is that  we have computed $\pi_{A_1 \cup C}^*(z)$ whenever ${\tt max}(C)=\gamma$ [or $C = \emptyset$, an option needed below] and $z$ is an item of type ${\tt min}(A_1 \cup C)$ which is a subset
