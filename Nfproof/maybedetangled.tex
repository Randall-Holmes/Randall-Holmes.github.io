...
\item[2/27/2024:]  Tightened up language in the description of augmentations of supports.
...
\item and for each $\epsilon$ such that $S_\epsilon$ is $(N_\beta,A)$ and $N$ is a near-litter, and $N^\circ=f_{\gamma,\beta}(x,T)$ for $-1<\gamma<{\tt min}(A_1)$, for $x\in \tau_\gamma$, we give a description of how all values $S_t$ with $\epsilon$ the first term of the sequence $t$ are selected;  choose an augmentation
$T^+$, and assign values $S^+_{[\epsilon]+u} = T^+_{u}$ for each $u$ in the domain of $T^+$ ($[\epsilon]+u$ being the result of prepending $\epsilon$ to the sequence $u$), then choose a $[\chi_0]$ dominating all elements of the domain of $T^+$ and assign further values  $S^+_{[\epsilon]+[\chi]} = (z,B)$, where $\chi>\chi_0$, $(z,B)$ does not occur as a value of $S^+$ with index less than $[\epsilon]+[\chi_0]$,
