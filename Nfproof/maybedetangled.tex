...
a descending sequence starting with $\epsilon_1$, and so forth.  Now look at the items $\tau_\delta^+$'s corresponding to these indices in $S^+$:  the values of $\iota_*^+$ at these items must decrease strictly, which is impossible.
