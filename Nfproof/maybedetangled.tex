...
By fiat, we state that $\emptyset$ is the only $-1$-support.

We define the action of a $\beta$-allowable permutation $\pi$ on a $\beta$-support $S$:  if $S_\epsilon = (x,A)$, $\pi[S]_\epsilon = (\pi_A(x),A)$.  In the case of $-1$-supports, $\pi[\emptyset]=\emptyset$.

An element $x$ of $\tau^*_\beta$ ($\beta \geq 0$) has $\beta$-support $S$ iff for every $\beta$-allowable permutation $\pi$, if $\pi[S] = S$ then $\pi(x)=x$.  We say that  an element $x$ of $\tau^*_\beta$ which has a $\beta$-support is $\beta$-symmetric. Every element of $\tau_{-1}$ has $\emptyset$ as a support (by fiat).
...
The well-ordering $<_\alpha^+$ of $\tau_\alpha^+$ must satisfy the condition that for each $(x,S) \in \tau_\alpha^+$, for each $(z,A) \in {\tt rng}(S)$, $\iota_*^+(y,T) <_\alpha^+ \iota_*^+(x,S)$ must hold [where $\iota_*^+$ computes ordinal position in any $\leq^+_\beta$] if $f_{\beta,\gamma}(y,T) = L$
where $L$ is a litter which meets $z \cap \tau_{-1}$.

There is no obstruction to enforcing this condition, because there are many elements of $\tau_{\alpha}^+$ for which there is no such obligation, which can be used to fill gaps, as it were: a support condition is constrained to have position in its native order later than the position of a small collection of support conditions of lower type.  
...
If $\gamma=-1$, we need to compute $\pi^*_{A_1\cup \{-1\}}(x)$, for which it is sufficient to compute $\pi^*_{A_1}(L_{{\tt min}(A_1)})$, where $L$ is the litter containing $x$.  For this case, the inductive hypothesis that we know how to deal with smaller index sets $A$ suffices.  Note that when the size of the index set is small enough, all items become flexible.
...
This hypothesis is that  we have computed $\pi_{A_1 \cup C}^*(z)$ whenever ${\tt max}(C)=\gamma$ and $z$ is an item of type ${\tt min}(C)$ which is a subset
of the image under an $f$ map of a $(y,T)$ with $\iota^+_*((y,T)) < \iota^+_*((x,S))$.  The condition on the construction of the orders
$\leq^+_\beta$ ensures that this implies that we can compute $\pi_{A_1 \cup C}^*[S]$.
...
we know the image or preimage under $\pi_{A_1 \cup B}^*$ of a typed near-litter which includes it).  Where we do not have this information, we can choose
...
It remains to show that if $S$ and $T$ are supports, and $S^*=T^*$, there is an allowable permutation $\pi$ such that $\pi[S]=T$.
