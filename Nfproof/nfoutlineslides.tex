...
\date{Given 6/18/2021 at BEST (up to p. 25), updated 6/19}
...
Traditional terminology which I might slip into if I am not careful is to, when $x$ is an atom in the clan $\tau^0_A$ belonging to the litter $L$, to refer to $\Pi_A([L])$ as the {\em parent\/} of $x$, or of $L$, or for that matter of $[L]$.
...
An $A$-allowable permutation is a permutation of atoms whose action fixes each $\tau^0_B$, fixes each $K_B$ (so it always maps litters to near-litters in the same clan), and fixes $\Pi_B$ for $B \ll A$ and (if $A$ is nonempty) for $B=A$.  An $\emptyset$-allowable permutation is simply called an allowable permutation.
...
satisfies $\pi(X)=X$.  A strict $A$-support is one for which if its domain meets $\tau^i_B$, either $B=A_1$ or there is $i$ such that $B_i=A$ (so certainly $B \ll A$ or $B=A$).

\end{slide}

\begin{slide}

An object $X$ has strong $A$-support $S$ iff $X$ has $A$-support $S$ and $S$ is a strict $A$-support and each atom in $S$ belongs to a near-litter in $S$ preceding it in $S$ and for each near litter $N \in S$ belonging to $\tau^1_B$ for $B \ll A$ or $B=A$, the segment of $S$
before $N$ includes  a strict $C$-support of $\Pi_B([N])$ where  $\Pi_B([N])\in \tau^i_C$ for $i=0,2$, unless $C=A_1$.
...
$\tau^2_A$ is the power set of $\tau^1_A$ in the FM model determined by $A$-allowable permutations (this is strongly suggested by the way it is defined) and so is the double power set of $\tau^0_A$.  We make the claim to be verified that the subsets of $\tau^1_A$ with supports are the same as the subsets with $A$-supports, and in fact the same as those with strict $A$-supports, so in fact $\tau^2_A$ is the power set of $\tau^1_A$ in the FM model determined by all allowable permutations.  Thus the old notation for this set was ${\cal P}^2_*({\tt clan}[A])$.
...
It isn't clear that I will use formats in this version  of the argument.

\end{slide}

\begin{slide}

We develop a representation $X \in \tau^2_A$.  Choose a nice support $S$ of $X$.  For each element $Y$ of $X$, present a nice support $S_Y$ of $Y$ which has
$S$ as an initial segment, and which includes no elements of $\tau^1_{A_1}$ other than atoms.  We claim that the collection of coding functions $\chi_{Y,S_Y}$ we use can be taken to be
of size $<\mu$, as they are determinable by suitably amalgamating coding functions for parents of litters in $\tau^1_A$ (taken from covering sets of size $<\mu$) and reference information about inclusion or exclusion of atoms and litters).

We claim that $X$ is completely determined by two items, the support $S$ and the collection of coding functions $\chi_{Y,S_Y}$.  The way this works is that
$X$ is the set of all $\chi{Y,S_Y}(T)$ where $S$ is an initial segment of $T$, for $Y \in X$.

Because the collection of coding functions $\chi_{Y,S_Y}$ is of size $<\mu$ and $\mu$ is strong limit, the collection of sets of such coding functions is also of size $<\mu$.  The set of supports is of size $\mu$.  So there are at most $\mu$ elements of  $\tau^2_A$ and it is obvious that there are at least $\mu$ such elements.
