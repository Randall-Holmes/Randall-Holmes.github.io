...
\item  $\iota^+_*(x) <\iota_*(L)$ [and so for any $N \sim L$, $\iota^+_*(x) <\iota_*(N)$], 
...
The basic order of the construction of $\pi$ is that the value of $(\pi^+_A)^i``x$ for $i=\pm 1$ is computed at each atom or near-litter $x$ assuming that the values have already been computed for $(\pi^+_B)^j``y$ for each $i = \pm1$ where $\iota_*(y)<\iota_*(x)$ and $A \subseteq B$ or $B=A_1$ [of course, this is equivalent to computing
...
Note at this point that we can already see that if we have computed $\pi^{+*}_{A}(x)$, then it will always be the case either that $\{x\}$ is in the domain of $\pi^0_A$ or that we have previously computed $\pi^*_A(L_{{\tt min}(A)})$ where $L$ is the litter containing $x$.
...
of the  image under an $f$ map of a $(y,T)$ with $\iota^+_*((y,T)) < \iota^+_*((x,S))$.  The conditions on the construction of the indexings
$\iota^+_\beta$ ensure that this implies that we can compute $\pi_{A_1 \cup \{\gamma\}}^*[S]$ and $(\pi_{A_1 \cup \{\gamma\}}^{-1})^*[S]$ 
\end{comment}

This hypothesis is that  we have computed $\pi_{A_1 \cup C}^*(z)$ and $(\pi_{A_1 \cup C}^{-1})^*(z)$ whenever ${\tt max}(C)=\gamma$ [or $C = \emptyset$, an option needed below] and $z$ is an item of type ${\tt min}(A_1 \cup C)$ with $\iota_*(z \cap \tau_{-1}) < \iota^+_*((x,S)) < \iota_*(L)$.  The conditions on the construction of the indexings
$\iota^+_\beta$ ensure that this implies that we can compute $\pi_{A_1 \cup \{\gamma\}}^*[S]$ and $(\pi_{A_1 \cup \{\gamma\}}^{-1})^*[S]$ 
...
such that $S_{F(\delta)} = (T^{\uparrow A_1})_\delta$ for each $\delta$ in the domain of $T$, or 1 if there is no such $F$ (the usefulness of 1 as a dummy being that it is not a function).
