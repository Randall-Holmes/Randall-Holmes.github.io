...
\author{M. Randall Holmes and Holly Sky Wilshaw}
...
The first author showed the equiconsistency of NF with the quite bizarre system TTT (tangled type theory) in 1995, which gave a possible approach to a consistency proof.  Since 2010, the first author has been attempting to write out proofs, first of the existence of a tangled web of cardinals in Mac Lane set theory [an indication of this approach is given below] and then directly of the existence of a model of tangled type theory.  The second author (with the assistance of others initially)
has formally verified the proof of the first author of the existence of a model of TTT, and so of the consistency of New Foundations, in the Lean proof verification system, and in the process has suggested minor corrections and considerable formal improvements to the argument originally proposed by the first author.
...
The strict well-ordering $<_L$ is defined in the obvious way.  {\bf NOTE:  this well-ordering is used in only one place (in the proof of Freedom of Action) to make it clear that part of the construction there is quite concrete and doesn't need choice.  As the remarks there suggest, its use is optional, and it is not used in the formalization.  I do think a nice fine point is made by its use in the paper.}
...
\begin{description}
\item[cardinality of the collection of near-litters:]  Note that there are $\mu$ near-litters, because the cofinality of $\mu$ is at least $\kappa$.
...
a strong limit cardinal of cofinality $\geq\alpha$.  We define a sequence of cardinals and a sequence of functions $f$ indexed by ordinals.
...
We also provide a well-ordering $\leq^+_\gamma$ of order type $\mu$ of $\tau_\gamma^+$ ($-1 \leq \gamma <\alpha$), by postulating an injection $\iota^+_\gamma$ from $\tau_\gamma^+$ into $\mu$ (it does not need to be onto) and defining $x \leq^+_\gamma y$ as $\iota^+_\gamma(x) \leq \iota^+_\gamma(y)$.   There are some hypotheses of the recursion about $\iota^+_\gamma$ which are stated below.  
...
We define for each $\beta,\gamma <\alpha$ a function $f_{\beta,\gamma}$ (whose definition does not actually depend on $\alpha$:  it will be the same at every stage).  $f_{\beta,\gamma}$ is an injection from $\tau_\beta^+$ into the set of litters, with range included in ${\bf X}_{\beta,\gamma}$ to ensure that distinct $f$ maps have disjoint ranges.

\begin{description}
\item [the definition of the $f$ maps (crucially important):]  When we define $f_{\beta,\gamma}(x)$, we presume that we have already defined it for $y <^+_\beta x$.
We define $f_{\beta,\gamma}(x)$ as $L$, where $\iota(L)$ is minimal such that 

\begin{enumerate}
\item $L \in {\bf X}_{\beta,\gamma}$, 
...
\item[Definition (pre-extensional, distinguished extension):]  We define the notion of {\em pre-extensional\/} element of $\tau^*_\beta$ ($\beta \in \alpha$).   An element $x$ of $\tau^*_\beta$ is pre-extensional iff there is a $\gamma<\beta$ such that (1) $x \cap \tau^*_\gamma \subseteq \tau_\gamma$, and (2) $\gamma=-1$ if
...
\item[Definition ($A$ maps and $A^{-1}$):] For any $\delta \in \alpha$ and nonempty subset $a$ of type $\gamma \neq \delta$, we define $A_\delta(a)$ as $$\{N_\delta:(\exists x \in a:\exists S:N \sim f_{\gamma,\delta}(x,S))\}.$$  For any nonempty subset of type $\delta$ there is at most one subset $y$ of any type such that $A_\delta(y)=x$.  There cannot be more than one such $y$ in any given type because the $f$ maps are injective.  There cannot be more than one such $y$ in different types because the ranges of $f$ maps with distinct index pairs are disjoint.   We use the notation $A^{-1}(y)$ for this set if it exists, defining a very partial function $A^{-1}$ on nonempoty subsets of types.
...
\item[supports defined:]  Where $0\leq\beta \leq \alpha$, a {\em $\beta$-support\/} is defined as a function from a small ordinal to a collection of pairs $(x,A)$, where $A$ is a finite subset of $\lambda$  with maximum $\beta$ (note that it will not contain $-1$) and $x\subseteq \tau_{-1}$ is either a singleton or a near-litter.   We may write $S_\delta$ intead of $S(\delta)$.
...
By fiat, we state that there is one $-1$-support, the empty set.  {\bf NOTE: Something else is used as the support of an atom in the formalization, but in both cases an atom has only one support, and it is never actually used for anything.}
...
$x$ has cardinality 1, and $\pi^+_A``x \sim \pi^0_A(x)$ otherwise.
...
$\{x\}$ is not in the domain of $\pi^0_A$.  For $x \in L^-$ we define $\pi^{+*}_{A}(x)$ as $\pi_{L^-,M^-}(x)$.
...
In this way we have computed $\pi^*_A(L)^\circ$, which we call $M$.  Let $L^-$ be the largest subset of $L$ which does not include any $x$ such that $\{x\}$ is in the domain of $\pi^0_A$.  Let $M^-$ be the largest subset of $M$ which does not include any $x$ such that $\{x\}$ is in the domain of $\pi^0_A$.  We can define $\pi^*_A(L_{{\tt min}(A)})$ as the union of $M^-_{{\tt min}(A)}$ and all $\pi^*_A(\{x\}_{{\tt min}(A)})$ for $x \in L\setminus L^-$.

We further extend this to describe the action of $\pi^+_{A}$ on elements of $L$:
for each $x \in L$, if $\{x\}$ is in the domain of $\pi^0_A$, which maps it to $\{y\}$, $\pi^{+*}_{A}$ maps $x$ to $y$.   For $x \in L^-$ we define $\pi^*_A(x)$ as $\pi_{L^-,M^-}(x)$.
...
\item[Note:]  This is roughly how it looks in the formalization.  The first author believes that the entries $\Pi$ are redundant:  a pair $(\delta_1,\delta_2)$ such that $\pi_2(S_{\delta_1}) = \pi_2(S_{\delta_2}) = A$ and $\pi_1(S_{\delta_1})\sim N$ and $\pi_1(S_{\delta_2}) \subseteq \pi_1(S_{\delta_1}) \Delta N$ (where $(N,A) = S_\epsilon$) is a pair $(\delta_1,\delta_2)$ such that $S_{\delta_2}$ is of the form $(\{y\},A)$ with either $\epsilon \in \pi_3(S^*_{\delta_2})$ and $\delta_1 \not\in \pi_3(S^*_{\delta_2})$ or $\epsilon \not\in \pi_3(S^*_{\delta_2})$ and $\delta_1 \in \pi_3(S^*_{\delta_2})$:  all the information in the components labelled $\Pi$ is in a specification.  I can easily believe that adding this information to the component of the specification at $\epsilon$ makes the Lean proof easier, and I may find it convenient in the paper proof.
...
\item[Definition (coding functions):]  For any presupport $S$ and object $x$, we can define a function $\chi_{x,S}$ which sends $T=\pi[S]$ to $\pi(x)$ for every $T$ in the orbit of $S$ under
the action of allowable permutations.  We call such functions {\em coding functions\/}.  Note that if $\pi[S]=\pi'[S]$ then $(\pi^{-1}\circ \pi')[S]= S$, so 
$(\pi^{-1}\circ \pi')(x)= x$, so $\pi(x)=\pi'(x)$, ensuring that the map $\chi_{x,S}$ for which we gave an implicit definition is well defined.
...
\item[Definition (the specification of a support):]   For each $\eta$-support $S$ we define a combinatorial object $S^*$ [not actually unique]  which we call its {\em specification\/}. We will show below that what it specifies is the orbit in the action of allowable permutations on supports to which it belongs.

NOTE:  massive modification 2/8/2024:  the original language is preserved in a hidden comment.

We first define an extension of the support $S$ indexed by finite sequences of ordinals rather than single ordinals.

We use a modified lexicographic order on finite sequences of ordinals:  if $s$ and $t$ are finite sequences of ordinals, we say $s<t$ if, $i$ being the first index for which $s_i=t_i$ does not hold, either $s_i<t_i$ or $s_i$ is defined and $t_i$ is not.  Note that a sequence appears {\em before\/} its proper initial segments.

We say that $S^+$ is an augmentation of $S$ iff  $S^+$ is a function from finite sequences of ordinals to support conditions and
...
\item and for each $\epsilon$ such that $S_\epsilon$ is $(N_\beta,A)$ and $N$ is a near-litter, and $N^\circ=f_{\gamma,\beta}(x,T)$ for $-1<\gamma<{\tt min}(A_1)$, for $x\in \tau_\gamma$, we give a description of how all values $S^+_t$ with $\epsilon$ the first term of the sequence $t$ are selected;  choose an augmentation
$T^+$, and assign values $S^+_{[\epsilon]+u} = (T^+_{u})^{\uparrow \eta}$ for each $u$ in the domain of $T^+$ ($[\epsilon]+u$ being the result of prepending $\epsilon$ to the sequence $u$, and $\uparrow$ being used to correct the top elements of second projections of support conditions), then choose a $[\chi_0]$ dominating all elements of the domain of $T^+$ and assign further values  $S^+_{[\epsilon]+[\chi]} = (z,B)$, where $\chi>\chi_0$, $(z,B)$ does not occur as a value of $S^+$ with index less than $[\epsilon]+[\chi_0]$,
$z \cap \tau_{-1}$ is a singleton, and $z \cap \tau_{-1}$ is a subset of a small intersection or set difference $\pi_1(S^+_{\eta_1}) \cap \pi_1(S^+_{\eta_2})$ or $\pi_1(S^+_{\eta_1}) \setminus \pi_1(S^+_{\eta_2})$ with $\pi_1(S^+_{\eta_{i}})=B$ and $\eta_i<[\epsilon]+[\chi_0]$ for $i=1,2$; all such $(z,B)$'s occur as such values at just one index $[\epsilon]+[\chi]$.  There are no other values of $S^+$ at finite sequences with first term $\epsilon$.  Augmentations of supports are not unique because there are multiple ways to
index such $(z,B)$'s.
...
Notice that the modified order on finite sequences ensures that we are inserting supports of inverses under $f$-maps of near-litter items before the near litter items, so in fact $S^+$ is a presupport, and clearly if $S$ is a support of any $x$, $S^+$ is a presupport of $x$.
...
a descending sequence starting with $\epsilon_0$, and so forth.  Now look at the items in $\tau_\delta^+$'s corresponding to these indices in $S^+$:  the values of $\iota_*^+$ at these items must decrease strictly, which is impossible.

We define for any $\eta$-support $S$ the notation $S^+_{(\gamma,\epsilon)}$ for $\gamma<\eta$ as signifying $$\{(\epsilon^*,(z,B)):{\tt max}(B)=\gamma \wedge (\epsilon^*,(z,B)^{\uparrow \eta}) \in S^+\wedge \epsilon^*<\epsilon\}.$$  Clearly this is a $\gamma$-presupport using modified lexicographic order on its domain.
...
\item  If $S^+_{\epsilon}$ is $(\{x\}_\beta,A)$, then $S^*_\epsilon$ is $(0,\beta,\Sigma,A)$ where  $\Sigma$ is the set of all $\delta$ such that $\pi_1(S^+_\epsilon) \subseteq \pi_1(S^+_\delta)$ and $\pi_2(S^+_\epsilon)=\pi_2(S^+_\delta)$ (this captures identical atoms and near litters containing the given atom)

\item  If $S^+_\epsilon$ is $(N_\beta,A)$ and $N$ is a near-litter, and either $|A|=1$ or $N^\circ$ is not in the range of any $f_{\gamma,\beta}$ for $\gamma<{\tt min}(A_1)$, then $S^*_\epsilon$ is $(1,\beta,\Sigma,A)$  where $\Sigma$ is the set of all $\delta$ such that $\pi_1(S^+_\epsilon)\sim\pi_1(S^+_{\delta})$   and $\pi_2(S^+_\epsilon)=\pi_2(S^+_\delta)$.

\item  If $S^+_\epsilon$ is $(N_\beta,A)$ and $N$ is a near-litter, and $N^\circ=f_{\gamma,\beta}(x,T)$ for $-1<\gamma<{\tt min}(A_1)$, for $x\in \tau_\gamma$ then 
$S^*_\epsilon$ is $(2,\beta,\chi_{x,S^+_{(\gamma,\epsilon)}},A)$.

\item  If $S^+_\epsilon$ is $(N_\beta,A)$ and $N$ is a near-litter, and $N^\circ=f_{-1,\beta}(x,\emptyset)$  then $S^*_\epsilon$ is $(3,\beta,\Sigma,A)$, where  $\Sigma$ is the set of all $\delta$ such that $S^+_\delta$ is $(\{x\}_{{\tt min}(A_1)},{\tt min}(A_1))$.
...
If we have $S^+_\epsilon = (\{x\}_\beta,A)$, we will have $T^+_\epsilon = (\{y\}_\beta,A)$ for some $y$, and we will set $\pi^0_A(\{x\}_\beta) = \{y\}_\beta$ as part of the construction of the local bijection to be used.  We can fill the orbit in a way compatible with our requirements, ensuring that no atom in a near-litter in one of the augmented supports or in the domain of the approximation as constructed so far is mapped to or from an atom in a manifestly wrong near-litter;  since there are a small number of such conditions to meet there can be no obstruction.

If we have $S^+_\epsilon = ((M_\beta,A)$ for $M$ a near litter and either $|A|=1$ or $M^\circ$ is not in the range of any $f_{\gamma,\beta}$ for $\gamma<{\tt min}(A_1)$, then $T^+_\epsilon = (N_\beta,A)$ for $N$ a near litter, with analogous properties, and we set $\pi^0_A(M^\circ_\beta) = N^\circ_\beta$  as part of the data for application of the Freedom of Action Theorem [actually, we set $\pi^0_A((M^-_\beta) = N^-_\beta$  for suitable subsets of $M^-$ and $N^-$, excluding any atoms in $M$ or $N$ that are assigned values;  this adjustment can be made at the end of the process].  Again, we fill in orbits of flexible near-litters, respecting conditions imposed by the augmented supports we start with and the approximation as defined so far.

If we have $S^+_\epsilon= (M_\beta,A)$ for $M$ a near litter with $M^\circ = f_{\gamma,\beta}(x,U)$, where $-1<\gamma<{\tt min}(A_1)$,
then $S^*_\epsilon$ is $(2,\beta,\chi_{x,S^+_{(\gamma,\epsilon)}},A)$ and where $T^+_\epsilon=(N_\beta,A)$, $T^*_\epsilon$ is $(2,\beta,\chi_{y,T^+_{(\gamma,\epsilon)}},A),$, where $N^\circ = f_{\gamma,\beta}(y,V)$ and the component specifications and coding functions are the same.

We use the inductive hypotheses that we have already computed full orbits of the entries in the local bijection associated with indices in $S^+$ preceding $\epsilon$:  thus we have already computed a $\gamma$-approximation sending $U^+$ to $V^+$ [and acting correctly on atoms appended after data from $U^+$ and $V^+$ in the respective computations of $S^+$ and $T^+$] and so the map constructed by the Freedom of Action theorem from the final approximation is already guaranteed to send $M_\beta$ to the right place up to nearness of litters.  [NOTE:  Note that this suggests that our original use of coding functions based on $U$ and $V$ rather than 
$S^+_{(\gamma,\epsilon)}$ and $T^+_{(\gamma,\epsilon)}$ may actually work.]
