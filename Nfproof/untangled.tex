...
\item[6/10/2022:]  Lifted some fog about how the argument for the axiom of union in the model of TTT works;  the language is still provisional but contains the key to why it works.  It does work as described in the previous version, but there was a gap rather larger than one should expect the reader to be able to fill.  The document still needs a detailed account of each of the three cases in the argument where permutations are constructed by matching of supports to build a local bijection.
...
The final move is to argue that there are $<\mu$ orbits in the $\alpha$-allowable permutations which contain strong supports.  The idea is that the orbit in which a support lies
is completely determined by a certain amount of combinatorial information, similarly to what happened in type 0 but a bit more complex.  The orbit is specified if we know the second and third components of each item (taken from $\lambda$ items in each case, the first and second components of the first item, and whether the third component is a singleton or a litter.  If this is a singleton, we want to know the position in the support of a litter containing it (which will be present).  If this is a litter and its local cardinal is an image under an $f$ map, we can extract from information about the preceding part of the support order a subsupport which is an index-raised version of a strong support for the inverse image of the local cardinal under the $f$ map:  as part of our specification, we take the coding function which generates that inverse image from that subsupport.
...
$((\beta,-1,X),A)$ in a way that we describe.  If $x=\{y\}$, we replace $x$ with $\{\delta\}$, where $\delta$ is the position of a typed near-litter containing $y$ in the obvious sense.  If $x$ is a near litter which does not belong to any $f_{\gamma,\beta}(y)$ with $\gamma<{\tt min}(A_1)$, then 
$X= \emptyset$.  If $x \in f_{\gamma,\beta}(y)$ with $\gamma<{\tt min}(A_1)$, then we extract the maximal strong support $T$ of $y$ such that
...
Suppose that we are considering the existence of $\{x : \phi^s\}$, where $\phi$ is a formula of the language of TST with $\in$ translated as $E$, and $s$ is a strictly increasing sequence of types.  The truth value of each subformula of $\phi$ will be preserved if we replace each $u$ of type $s(i)$ with $\pi_{A_{s,i}}(u)$, where  $A_{s,i}$ is the set of all $s_k$ for $i \leq k \leq j+1$, where $x$ is of type $s(j)$:  $\pi_{A_{s,i}}(x) E  \pi_{A_{s,i+1}}(y)$ is equivalent to $(\pi_{A_{s,i+1}})_{s(i)}(x) E \pi_{A_{s,i+1}}(y)$, which is equivalent to $xEy$ by the observation above. The formula $\phi$ will contain various parameters $a_i$ of types $s(n_i)$ and it is then evident that the set $\{x : \phi^s\}$ will be fixed by any $s(j+1)$-allowable permutation $\pi$ such that $\pi_{A_{s,n_i}}$ fixes $a_i$ for each $i$.  But this means that
$(s(j+1),s(j),\{x : \phi^s\})$ is symmetric and belongs to type $s(j+1)$, as long as no parameter is of type higher than $s(j+1)$ (the predicativity condition):  one can construct a support by merging the supports of the parameters $a_i$ (with index raising), and one obtains a $s(j+1)$-support.
...
Fix $g \in G$.  The element $g$ has a strong $\gamma$-support $T$ which extends $(S_\beta)_\gamma$.  

Our strategy is to attempt construction by freedom of action technology of an $\alpha$-allowable permutation $\pi^*$ which fixes
$S \setminus S_\beta$ and agrees with $\pi$ on $T$ (always via appropriate derivatives).  This permutation $\pi^*$ will then fix $(\alpha,\beta,\{(\beta,\gamma,\{g\}):g \in G\})$
and at the same time $(\pi^*_\beta)\gamma$ will send $g$ to $\pi_\gamma(g)$  from which it follows that $\pi_\gamma(g) \in G$ (and the same follows for $\pi^{-1}$ by the same argument) so $\pi$ fixes $(\beta,\gamma,G)$ as desired.

The strategy for construction of $\pi^*$ is that it fixes atomic items in $S\setminus S_\beta$ and sends both elements of the orbit under $\pi$ of atomic items appearing in $T$ and the orbit under $\pi$ of exceptions of derivatives of $\pi$ which lie in litters in $T$ to the values expected for $\pi$ (and its derivatives).  If this works, it has the right effects.  To get it to work requires an additional closure condition, which deals with the problem of type $\gamma$ litters in $T$ which may not be $\{\alpha,\beta,\gamma\}$-flexible:  for any item $(\{\alpha,\beta,\delta\},u)$ in the domain of the support, where
$x$ is a litter which is not $\{\alpha,\beta,\delta\}$-flexible (that is, $x=f_{\epsilon,\delta}(y)$ for some $\epsilon<\beta$ and $y\in \tau_\epsilon$) we select an $\epsilon$-support
$U$ of $y$, and for each $(u,A)\in U$ we map $(A\cup \{\alpha,\beta\},u)$ to $\pi_A(u)$;  we iterate this as necessary, and of course add exceptions of $\pi$ which fall in added litters as required.  After this extension, the freedom of action theorem allows us to extend the local bijection described in the usual way (using $\chi_{\{\alpha,\beta,\delta\}}$ maps to set desired values at typed litters of maximal type in the extension of $T$ as required), both on $S \setminus S_\beta$ and on $T^{\{\alpha,\beta\}}$ extended in the way we have described, and it is clear that there is no interaction between the extension of $T$ and $S \setminus S_\beta$.
