...
We argue that there are $<\mu$ orbits in the $\alpha$-allowable permutations which contain strong supports.  The idea is that the orbit in which a support lies
...
It should also be clear that the image under an allowable permutation of a support has the same specification as the input support.
...
We build a support $S+T_b^{\{\alpha\}}$ and a support $S+U^{\{\alpha\}}$ with parallel structure by appending $T_b$ (respectively $U$) to $S$ then removing all but the first occurrence of each repeated item.   These two supports have the same specification (because of the identity of $S$ and $S$
and the fact that $U$ is in the orbit of $T_b$), so they are in the same orbit in the allowable permutations:  find $\pi$ whose action sends $S+T_b^{\{\alpha\}}$ to $S+U^{\{\alpha\}}$ and you have found $\pi$ which fixes $X$ and has $\pi_\beta(b)=c$.
...
There is an obstruction to doing this:  we describe the obstruction and the solution.  The obstruction arises if any element of $T$ is of the form
$(u,\{\gamma\})$ with $u$ a litter which is not $\{\beta,\gamma\}$-flexible (that is, for which there is $\delta<\beta$ with $u \in f_{\delta,\gamma}(v)$).
The solution is to extend $T$ to a set $T^*$ closed under the condition that if $(u,\delta) \in T^*$ and there is $\epsilon<\beta$ with
$u \in f_{\epsilon,\delta}(v)$, then an $\epsilon$-strong support for $v$ is included in $T^*$;  the extension is by adding strong supports (using designated supports to do this, so that $T^*$ is well-ordered in the usual way) as needed.  $T^*$ is not necessarily a a support (because the extended type indices in its domain do not all have the same largest element), but $(T^*)^{\{\alpha,\beta\}}$ is.

The local bijection that we construct sends each atom appearing in $S$ to itself, and each atom appearing in $T^*$ to its image under the appropriate derivative of $\pi$, along with each element of its orbit under $\pi$, and each exception of a derivative of $\pi$ which appears in a litter in $T^*$ or is mapped into such a litter
to its image under that derivative, and the same for each element of its orbit under $\pi$.  Further, the $\chi_{\{\alpha,\beta,\delta\}}$ maps that we use enforce that typed litters of maximum type below $\beta$ appearing in $T^*$ are sent to their images under $\pi$.   One argues by induction that all typed litters in $T^*$ can be mapped to their images under $\pi$, because each one which is not flexible has been supplied with a support along which the computation can be carried out.  The actions driven by $T^*$ do not conflict with the actions on $S$ (because the action of $\pi$ fixes $S_\beta$ and the rest of $S$ involves type indices not involved in $T^*$), and the conditions described above are achieved.
