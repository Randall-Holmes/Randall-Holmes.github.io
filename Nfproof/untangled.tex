...
\item[5/22/2022, evening:]  This incorporates some additional remarks from Saturday's meeting and corrects a typo that a student pointed out.
...
Note that the definition presumes that $\pi_\beta$, which is defined on all $\beta$-codes, sends elements of $\tau_\beta$ to elements of $\tau_\beta$.  We will point out when symmetry is defined why any image of a symmetric code under an allowable permutation is symmetric.
...
\item[Applying permutations to objects with support:]  Quite standard techniques show that if $\pi$ is an $\alpha$-allowable permutation and $X \in \tau_\alpha$ has $\alpha$-support $S$, then $\pi(X)$ has $\alpha$-support $\pi[S]$.   It follows from this that an allowable permutation on $\alpha$-codes restricts to an allowable permutation on $\tau_\alpha$, as we presumed in the definition of allowable permutation.
