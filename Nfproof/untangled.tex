...
$S^*_a$ is $((\beta,-1,\chi_{x,S_{<a}^\gamma},A)$:  the component replacing $N$ is the coding function with largest possible domain taken from $S$ which yields $x$.
...
then $S^*_a$ is $((\beta,-1,\chi_{x,S_{<a}^\gamma},A)$ and $T^*_a$ is $((\beta,-1,\chi_{x,T_{<a}^\gamma},A).$

If all earlier items in $S$ are mapped by a fixed allowable permutation $\pi_0$ to the corresponding items in $T$, then
$$\pi_0(x) = \pi_0(\chi_{x,S_{<a}^\gamma}(S_{<a}^\gamma) $$ $$ = \chi_{x,S_{<a}^\gamma}(\pi_0[S_{<a}^\gamma])$$ $$ = \chi_{x,S_{<a}^\gamma}(T_{<a}^\gamma) $$ $$= \chi_{y,T_{<a}^\gamma}(T_{<a}^\gamma)=y$$
