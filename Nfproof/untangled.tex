...
We can formally define the order $S^\circ$:  the domain of $S^\circ$ consists of all elements $((\beta,-1,x),A,\delta)$ for which $x$ is a singleton or a litter [we call these preserved elements of the domain]  and elements $((\beta,-1,y),A,\delta)$ such that for some $x$, $((\beta,-1,x),A,\delta)$ is in the domain, $x$ is a near-litter and not a litter, 
and either $y=x^\circ$ or $y = \{z\} \subseteq x\Delta x^\circ$ (we call these new elements of the domain:  notice that nothing prevents a new element from also being a preserved element).  We refer to $((\beta,-1,x),A,\delta)$ as the archetype of $((\beta,-1,y),A,\delta)$.
