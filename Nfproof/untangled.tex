...
$\pi_{A\cup \{-1\}}$ extends $\pi^0_A$ for each $A$, and satisfying a further technical condition:    the permutation $\pi$ obtained from $\pi^0$ has no exceptions which are not elements of the domain of $\pi^0$.
...
For each nonempty finite subset $A$ of $\lambda$, we specify a permutation $\chi_A$ of local cardinals $[L]$ which are not of the form $f_{\gamma,\beta}(y)$ for any $\gamma,\beta <{\tt min}(A)$ and $y \in \tau_\gamma$.
