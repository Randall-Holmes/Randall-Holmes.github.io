...
We stipulate that $(\alpha,\beta,\{b\})$ is an element of $\tau_\alpha$ (a symmetric code, and clearly the representative of its equivalence class) for each $b \in \tau_\beta$, $\beta<\alpha$.  This will have to be verified when symmetry is defined.
...
\item[Observation (cardinality of the set of near-litters):]  It is a useful observation that because $\mu$ has cofinality $>\kappa$, there are no more than $\mu$ (and so exactly $\mu$) near-litters,
...
\item[Observation (any support can be extended to a strong support):]  We describe the process of extending a support to a strong support.  Before each typed singleton element of the domain, insert the appropriate typed litter (removing an extra copy of it if it occurred later in the order).  This will only need to be done once for each typed singleton element.
...
The third components in the elements of support domains seem actually to be necessary to ensure that strong supports can always be produced.  What the third component is doing is providing a comment on the index of the sub-support the element is required for.
...
$\pi_{A\cup \{-1\}}$ extends $\pi^0_A$ for each $A$, and satisfying a further technical condition:    the permutation $\pi$ obtained from $\pi^0$ has no exceptions of its derivatives other than those determined by elements of the domains of maps $\pi^0_A$:  $(A,x)$ being in the domain of $\pi^0$ allows (but does not require) $x$ to be an exception of $\pi_{A \cup \{-1\}}$.
...
was whether existence of a Specker tree of infinite rank is consistent.  It is known that in NF with the Axiom of Counting the Specker tree of the cardinality of the universe is of infinite rank.  Our results can be used to show that Specker trees of infinite rank are consistent in ZFA (this takes a little work, using the way that internal type representations unfold in TTT).  We are confident that our permutation methods can be adapted to ZFC using forcing in standard ways to show that Specker trees of infinite rank can exist in ZF.
