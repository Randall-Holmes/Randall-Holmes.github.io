...
\item[6/4/2022, afternoon Cambridge time:]  Explicitly described the procedures for extending supports to strong supports.
...
Let $\lambda$ be a limit ordinal.  Elements of $\lambda$ will be indices of types in the model of tangled type theory.  $-1$ is an index of an additional type in the structure we are using to build the model.  Elements of $\lambda \cup \{-1\}$ are called type indices;  type indices other than $-1$ are called proper type indices.  The order on the type indices is the obvious one in which $-1$ is less than the elements of $\lambda$.
...
\begin{description}
\item[Definition (codes):]  We suppose for $\beta<\alpha$ a proper type index that a $\beta$-code has already been defined as a triple $(\beta,\gamma,G)$ where $\gamma<\beta$ and $G \subseteq \tau_{\gamma}$. 

A $\alpha$-code is defined now as a triple $(\alpha,\beta,B)$ where $\beta<\alpha$ and $B \subseteq \tau_{\beta}$.   
...
\item[Definition ($f$ maps):]  All codes $(\beta,-1,N)$ are stipulated to be elements of $\tau_\beta$ (this will be verified as following from inductive hypotheses given in more detail later).   We refer to such objects as typed near-litters.   On each type $\delta$, $-1 \leq \delta <\alpha$, we have chosen a well-ordering $\leq_\delta$ of order type $\mu$ (with corresponding strict well-ordering $<_\delta$).  We define $\iota(x)$ for $x \in \tau_\beta$ as the order type of $\leq_\beta$ restricted to $\{y \in \tau_\beta:y <_\beta x\}$.  Where $\beta$ is a type index less than $\alpha$ and $\gamma$ is a proper type index distinct from $\beta$,  we define $f_{\beta,\gamma}(x)$ as the local cardinal $[N]$ of the third component of the first $(\gamma,-1,N)$ in $<_\gamma$ such that $N$ is a near-litter and $[N] \in X_{(\beta,\gamma)}$ and $\iota(\gamma,-1,M)>\iota(x)$ for each $M \in [N]$ and $[N] \neq f_{\beta,\gamma}(y)$ for any $y <_\beta x$.   Note that this definition will give the same result at any stage whose index dominates both $\beta$ and $\gamma$.

\item[Definition (alternative extension map):]  For any $\beta$-code $(\beta,\gamma,G)$ with $\beta \leq \alpha$ a proper type index and $\delta$ a proper type index distinct from $\gamma$ and less than $\beta$, we define
...
\item[Observations and definition of $A^{-1}$:]  Notice that since the ranges of $f_{\beta,\gamma}$ and $f_{\beta',\gamma'}$ are disjoint unless $\beta=\beta'$ and $\gamma=\gamma'$, it follows that the ranges of
...
$$(\forall Z \in \tau_\gamma: Z \in_{TTT} X \leftrightarrow Z \in_{TTT} Y) \rightarrow X = Y).$$  This should be evident from the method of construction.
...
\item[Stipulation:]  We postulate as part of the hypotheses of the recursion that $(\beta,\gamma,\{x\}) \in \tau_\beta$ for any $x \in \tau_\gamma$, for any $\gamma<\beta<\alpha$.  This will be verifiable from a more complete description of our assumptions about $\tau_\beta$ for $\beta<\alpha$ which is given below.
...
A $-1$-allowable permutation is a permutation $\pi$ of $\tau_{-1}$ with the property that for any near-litter $N$, $\pi``N$ is a near-litter.
...
\item[Definition (support and symmetry):]  An $\beta$-support is a well-ordering of a small set of triples $(X,A,\delta)$, where $X$ is a $\gamma$-code for some $\gamma<\beta$ [which may be different for different elements of the support] of the form $(\gamma,-1,x)$ where $x$ is either a singleton or a near-litter, $A$ is an extended type index with maximum $\beta$ and minimum $\gamma$, and $\delta$ is a proper type index $\leq \beta$.
Further, we impose the condition that if $((\gamma,-1,x),A,\delta)$ and $((\gamma,-1,x'),A,\epsilon)$ belong to the domain of the support and $x,x'$ are near-litters, then $x$ and $x'$ are disjoint.
...
If $S$ is an $\beta$-support, we define $S^+$ as $$\{((x,A,\beta),(y,B,\beta)): (x,A,\beta), (y,B,\beta) \in S\},$$ the restriction of $S$ to triples with third component $\beta$.

We can then say that $S$ is a $\beta$-support of $X$ if $X$ is a $\beta$-code, $S$ is an $\beta$-support, and for any $\beta$-allowable permutation $\pi$, if $\pi[S^+]=S^+$ then $\pi(x)=x$.  In some sense the items in the support with third components less than $\alpha$ are fluff, but they do have uses as we will see.
...
\item[Definition:]  We say that a support $S'$ extends a support $S$ if ${\tt dom}(S) \subset {\tt dom}(S')$:  we might change the order in the extension process.
...
element $((\beta,-1,x),A,\delta)$ of the domain of $S$ for which $x$ is a near-litter and not a litter with $((\beta,-1,x^\circ),A,\delta)$ and $((\beta,-1,\{y\}),A,\delta)$ for each $y$ in the symmetric difference of $x$ and $x^\circ$.

We can formally define the order $S^\circ$:  the domain of $S^\circ$ consists of all elements $((\beta,-1,x),A,\delta)$ for which $x$ is a singleton or a litter [we call these preserved elements of the domain]  and elements $((\beta,-1,y),A,\delta)$ such that for some $x$, $((\beta,-1,x),A,\delta)$ is in the domain, $x$ is a litter and not a near-litter, 
and either $y=x^\circ$ or $y = \{z\} \subseteq \Delta x^\circ$ (we call these new elements of the domain:  notice that nothing prevents a new element from also being a preserved element).  We refer to $((\beta,-1,x),A,\delta)$ as the archetype of $((\beta,-1,y),A,\delta)$.
...
Any $\beta$-support $S$ can be converted to a support satisfying the first two conditions (and so any 0-support can be converted to a strong support).  First define $S^\circ$ as above, so we have a support consisting entirely of typed singletons and
typed litters.  The support $S'$ which we construct has as its domain elements domain elements of $S^\circ$, which we call preserved elements of the domain of $S'$, and elements $((\beta,-1,x),A,\delta)$ which have the property that there is $((\beta,-1,\{y\}),A,\delta)$ with $y \in x$ but not $((\beta,-1,x),A,\delta) <_{S^\circ} ((\beta,-1,\{y\}),A,\delta)$.  In this case, we say that $((\beta,-1,\{y\}),A,\delta)$ is an archetype of $((\beta,-1,x),A,\delta)$.  For any item $X$ in the domain which is preserved, $X'=X$.  For an $X$ which is new (and thus a typed litter) and also preserved, we let $X'$ be the minimum in $S^\circ$ of $X$ and all its archetypes.
For $X$ which is new and not preserved, we let $X'$ be the archetype of $X$.  If $X' \neq Y'$ and $X' \, S^\circ \, Y'$, we assert $X \, S' \, Y$.
The only situation where $X'=Y'$ and $X \neq Y$ is one in which one of them is a typed litter and the other is a typed singleton of an element belonging to the litter.  In this case, put the typed litter first in the order $S'$.  This completes the definition of the method of deriving a support satisfying the first two conditions.

Now suppose that for every $\beta<\alpha$ we have established that each $\beta$-support $S$ (which we may suppose wlog satisfies the first two conditions, as we can first apply the transformations described above) can be extended to a strong $\beta$-support $S'$, effectively
(given $S$, we have defined $S'$;  we also postulate an effective way of choosing strong supports for given objects of type $<\alpha$;  we do have the axiom of choice in the metatheory!).  Let $S$ be an $\alpha$-support (supposed wlog to satisfy the first two conditions):  we describe the construction of a strong $\alpha$-support $S'$ extending $S$.

The domain of $S'$ consists of elements of the domain of $S$ (which we call preserved elements of the domain) and for each element $((\beta,-1,x),A,\delta)$ for which $x$ is a litter and $f_{\gamma,\beta}^{-1}([x])$ is defined for some $\gamma<\delta$, the elements of the domain of $T^{A \setminus \{\beta\}}$ where $T$ is your favorite strong $\gamma$-support of $f_{\gamma,\beta}^{-1}([x])$:  these elements are called new elements and have $((\beta,-1,x),A,\delta)$ as an archetype.
Define $X'$ as $X$ if $X$ is preserved and not new, the minimum in $S$ of $X$ and all of its archetypes if $X$ is preserved and new, and the smallest of the archetypes of $X$ in the order $S$ if $X$ is new and not preserved.  If $X' \neq Y'$ and $X'\, S\, Y'$, we assert $X\, S'\, Y$.  If $X' = Y'$ and $X \neq Y$,
then if one of $X$ and $Y$ is preserved, it is later in the order $S'$, and otherwise $X$ and $Y$ are both elements of a support $T^{A \setminus \{\beta\}}$ chosen as above, and we provide that the order $T^{A \setminus \{\beta\}}$ agrees with the order $S'$ in this case.



\end{description}

The third components in the elements of support domains seem actually to be necessary to ensure that strong supports can always be produced [they avert infinite regress in the process of insertion of new supports in the last part of the procedure].  What the third component is doing is providing a comment on the index of the sub-support the element is required for.   There is another approach:  indexing the $f$ maps with $\alpha,\beta,\gamma$ instead of just $\beta,\gamma$ and requiring that ranges of $f$ maps with distinct indices be disjoint, which would require very straightforward modifications of the paper above, would ensure that we could tell the difference between insertions into an $\alpha$ support and insertions into a $\beta$-support for $\beta<\alpha$.  It seems that the added complexity is about the same with both approaches, and I have left it this way.  The approach used here also has the interesting effect that the extension over type $\gamma$ of a type $\alpha$ set is determined by its extension over type $\beta$ in a way which does not depend on $\alpha$;  it is interesting that this can be done.
...
The nasty recursive idea here is that we already know how to compute $\pi_{A \setminus \{\beta\} \cup \{\gamma\}}(y)$ from the embedded strong $\gamma$ support of $y$ because we assume as a hypothesis of the recursion that we already know how to carry out this computation for $\gamma$-supports, $\gamma<\alpha$.  The  local information we need about the $\gamma$ allowable permutation (the local bijection to be used and the relevant $\chi$ maps) is included in the information we are given initially about $\pi$ (though this data will be restricted in scope and have extended type indices modified when used on the $\gamma$-support, of course:  any data involving an extended type index $A$ has $A$ replaced with $A \cap (\gamma+1)$ [and eliminated if this is empty] and applied to calculation of values of the $\gamma$-allowable permutation $\pi_{A \setminus \{\beta\} \cup \{\gamma\}}$).  Notice that we certainly do know how to do it for 0-supports, because the recursive clause will never be invoked if $\alpha=0$:  the rest of the procedure tells us what to do.
