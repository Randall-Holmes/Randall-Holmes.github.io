...
Further, we impose the condition that if $((\gamma,-1,x),A,\delta)$ and $((\gamma,-1,x'),A,\epsilon)$ belong to the domain of the support and $x,x'$ are near-litters, then $x$ and $x'$ are equal or disjoint [our current arrangements may impose the need for essentially the same support element to appear with more than one third component.]
...
then if one of $X$ and $Y$ is preserved, we define it as later in the order $S'$, and otherwise $X$ and $Y$ are both elements of a support $T^{A \setminus \{\beta\}}$ chosen as above, and we provide that the order $T^{A \setminus \{\beta\}}$ agrees with the order $S'$ in this case.
...
$\pi_{A\cup \{-1\}}$ extends $\pi^0_A$ for each $A$, and satisfying a further technical condition:    the permutation $\pi$ obtained from $\pi^0$ has no exceptions of its derivatives which are not elements of the domain of $\pi^0$.
...
For each nonempty finite subset $A$ of $\lambda$, we specify a permutation $\chi_A$ of local cardinals $[L]$ which are not of the form $f_{\gamma,\beta}(y)$ for any $\gamma<{\tt min}(A)$ and $y \in \tau_\gamma$.
