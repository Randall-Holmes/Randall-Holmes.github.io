...
We use the notation $\pi_3$ for the third projection of a triple to extract extensions from codes.  We introduce the notation $\pi_3^*(x)$ for the sole element of $\pi_3(x)$ in the case where $\pi_3(x)$ is a singleton (in particular, a typed atom).
...
\item[Definition (order on a support):]  If $S$ is a $\beta$-support, we define the order $<_S$ on $S$ by $(x,A) <_S (y,B)$ iff the two conditions belong to $S$ as
$\iota_*(x)<\iota_*(y)$.
...
\item  The domain and range of $\pi^0_A$ are the same.  
...
If $x$ is $A$-flexible, $x=(\gamma,-1,N)$ where $N$ is a near-litter.  We have an $M \subseteq N^\circ$ at which $\pi^0_A(\gamma,-1,M)$ is defined.
For every $z \in N^{\circ} \setminus M$, $\pi^0_A(\gamma,-1,\{z\})$ is defined.  We define a convenient map $\pi_*$ on $N^\circ$, sending
$z$ to the element of $\pi_3(\pi^0_A(\gamma,-1,\{z\}))$ when this is defined, and otherwise to $\pi_{M,\pi_3(\pi^0_A(\gamma,-1,M))}(z)$.
We then define $\pi_A(\gamma,-1,N)$ as $(\gamma,-1,\pi_*``N)$.

We are left with the case where $[x] = f_{\delta,\gamma}(u)$ for some $u \in \tau_\delta$, with $\delta<{\tt min}(A_1)$.   In this case we know that
there is a support $T$ of $u$ such that all elements of $\{(v,C \cup (A \cap \gamma)):(v,C)\in T\}$ are $\leq (x,A)$ ($T$ being the designated $\delta$-support of $u$).  Thus we have already computed $\pi_{C \cup (A \cap \gamma)}(v)$ for each element of this set by hypothesis of the recursion.
Thus for each element $v$ of the $\delta$-support $T$ we have a desired value of a derivative of the $\delta$-permutation $\pi_{(A \cap \gamma) \cup \{\delta\}}$.  We extend these values to a $\delta$-approximation:  we have a value for each typed atom in $T$ and for each litter in $T$ with atoms in $T$ removed.  We need to extend this so that the domain and range of the approximation are the same.   To do this, we fill out orbits of typed atoms, the only constraint being that atoms in litters in $T$ must be mapped (by the approximation and its inverse) compatibly with the way the litters are supposed to be mapped.  Only countably many atoms for each atom in $T$ need to be added;  the constraints clearly can be met.  Extending the map on flexible near-litters presents no difficulty.  We get a $\delta$-approximation whose domain includes all typed atoms (including the new ones added) and apply the inductive hypothesis to get a $\delta$-permutation which it approximates.  We need to verify that it sends non-flexible near-litters in $T$ to the right values.
It certainly sends a non-flexible litter in $T$ to a near-litter with the correct local cardinal, because its appropriate derivative sends the preimage of that
local cardinal under the appropriate $f$ map to the right value, because its derivatives act correctly on a support of the preimage.  Now if it did not
send the non-flexible litter to the correct near-litter, this would imply existence of an exception of the appropriate derivative, and all exceptions of the $\delta$-permutation we have constructed which belong to (or do not belong to the litter in $T$ under consideration are recorded in the approximation and mapped consistently with our expectations about where the litter should  be mapped.   So in fact the action of the $\delta$-approximation we have constructed is the same on $T$ as we expect the action of $\pi_{(A \cap \gamma) \cup \{\delta\}}$, so we can apply the $\delta$-permutation we have constructed to $u$ to get the only possible value of $\pi_{(A \cap \gamma) \cup \{\delta\}}(u)$, and the inverse 

\end{description}

[This is not finished, but is also not part of the main line of development]
...
tagged version of $S_d$.  Note that in this situation, $\delta>\gamma$ always holds when an insertion is needed.  The resulting support will be a strong $\alpha$-support for $\{b\}_\alpha$;  note that it will contain no type $\alpha$ near-litters, there being no mechanism which would add them in our closure process.  This process terminates because in adding a support $S_d$ we are always adding things whose position in the designated order on their type is before the position of $d$ in its type.
...
$S_b$, but what we actually need is something like the specification of $T_b$ being computable from the specification of $S_b$.  There are $<\mu$ specifications for supports $S_b$ by the inductive hypothesis.   Any specification built the way we describe uses
an $S_b$ to start, which has one of $<\mu$ possible specifications.  Each further refinement involves choosing an $S_d$ (with one of $<\mu$ possible specifications) to insert 
at stated positions (describable with $<\kappa$ information; to see this, it is important to note that the inserted $S_d$ will not have links in its specification to material already appearing in the previous partial specification, apart from its link to an $f_{\gamma,\delta}(d))$; so all that is needed is the information as to where to insert it) to produce a further partial specification.  There will be $<\kappa$ such insertions.
There are $<\mu$ possible descriptions of such processes of insertion, including descriptions of actually ill-founded processes (of course, an actual construction of a $T_b$ will be well-founded).  So there are $<\mu$ possible specifications for supports $T_b$ constructed as above.
