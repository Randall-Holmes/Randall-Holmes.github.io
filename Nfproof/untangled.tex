...
We designate a pairwise disjoint family of subsets $X_{(\beta,\gamma)}$ of the set of local cardinals, each of cardinality $\mu$, indexed by ordered pairs $(\beta,\gamma)$ with $-1 \leq \beta <\lambda$ and $\gamma \in \lambda \setminus \{\beta\}$.
...
\item[Observations and definition of $A^{-1}$:]  Notice that since the ranges of $f_{\beta,\gamma}$ and $f_{\beta',\gamma'}$ are disjoint unless $\alpha=\alpha'$ and $\beta=\beta'$, it follows that the ranges of
...
For each proper type index $\beta$, $(\beta,\gamma,\emptyset) \equiv (\beta,\delta,D)$ iff $D=\emptyset$, and the representative member of this equivalence class is $(\beta,-1,\emptyset)$.
...
\item[Definition (notation for derivatives of a permutation):] We provide extended notation for the lower type permutations on which an $\beta$-structural permutation depends, for any proper type index $\beta \leq \alpha$.
...
$\gamma$-allowable, and further for any codes $X,Y$,  $X \equiv_\beta Y \leftrightarrow \pi(X) \equiv_\beta \pi(y)$.
