...
$u \in f_{\epsilon,\delta}(v)$, then an $\epsilon$-strong support for $v$ is included in $T^*$;  the extension is by adding strong supports (presumably designated ones) as needed.  $T^*$ is not necessarily a a support, but $(T^*)^{\{\alpha,\beta\}}$ is.

The local bijection that we construct sends each atom appearing in $S$ to itself, and each atom appearing in $T^*$ to its image under the appropriate derivative of $\pi$, along with each element of its orbit under $\pi$, and each exception of a derivative of $\pi$ which appears in a litter in $T^*$
to its image under that derivative, and the same for each element of its orbit under $\pi$.  Further, the $\chi_{\{\alpha,\beta,\delta\}}$ maps that we use enforce that typed litters of maximum type below $\beta$ appearing in $T^*$ are sent to their images under $\pi$.   The actions driven by $T^*$ do not conflict with the actions on $S$ (because $\pi$ fixes $S_\beta$ and the rest of $S$ involves type indices not involved in $T^*$), and the conditions described above are achieved.
