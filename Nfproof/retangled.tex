...
For each $b \in B$ there is a strong support $S_b$ chosen as above, from which the support $T_b$ can be computed as described above.  Let $S_b^-$ be the suborder of $S_b$ whose field is the designated support of $b$.  If $b' \in B$ is in the range of the same coding function $\chi_{b,S_b}$ as $b$, the details of our choice of designated supports show that $S_{b'}^-$ is $\pi[S_{b}^-]$ for some $\beta$-allowable $\pi$ with $\pi(b) = b'$.
