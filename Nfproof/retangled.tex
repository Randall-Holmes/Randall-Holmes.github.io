...
\item[Proof:]  For purposes of this proof we choose for every pair $(L,M)$ of co-small subsets of litters a bijection $\pi_{L,M}$ from $L$ to $M$.  This is not an application of AC:  it can be done concretely using the indexing of elements of litters, but the details are not important.  We further assume in this proof that any $\gamma$-allowable permutation for $\gamma<\beta$
which is constructed in the course of the proof using the indcutive hypthesis is constructed using the same $\pi_{L,M}$'s.
...
$\pi_\gamma(y)$, and we can then compute $(\pi_A(x)\cap \tau_{-1})^\circ=f_{\gamma,{\tt min}(A)}^{-1}(\pi'(y))$ and complete the calculation of $\pi_A(x)$ as above.  Recall that the same $\pi_{L,M}$'s are used in the calculation of $\pi'$.
