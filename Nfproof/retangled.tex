...
\item[3/31/2023:]  Some editing motivated by Sky Wilshaw's concern about bookkeeping difficulties he is encountering the proof of Freedom of Action having to do with enforcing the condition that the process of extension of approximations in the proof preserves injectivity.   The difficulty he is encountering seems difficult to describe and I may improve the language about it here.  I made various technical corrections through the entire text of the construction through the end of Freedom of Action, fixing various notational bugs.  There is some language about clarifying the inductive hypothesis in the way he needs, which doesn't satisfy me perfectly.
...
An element $x$ of $\tau^*_\beta$ has $\beta$-support $S$ iff for every $\beta$-allowable permutation $\pi$,  $\pi(x)=x$ if  for each $(u,A) \in S$,  $\pi_A(u) = u$.
