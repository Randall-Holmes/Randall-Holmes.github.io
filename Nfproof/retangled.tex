...
\item[11/17/2023:]  I am posting this version, which is still rather messy in the area of the gap being fixed, at the same time I post the new version in which the domain of the f maps includes support information.  I believe that as a paper document the new paper will now be more reliable, but this paper is still the original from which the Lean development has been done (though it must be said that the details of the Lean development, a construction in type theory, look quite different in places).

\item[11/28/2023:]  Updating this version in various ways to support the Lean work, which depends on approaches taken here.  Sky is working on translating things into terms of the new version {\tt maybedetangled}, which both of us prefer, but for the time being this one must be maintained.  This is an ongoing process;  I did a few edits today which I am posting mostly to have this exact note on record.
...
\begin{description}
\item[Construction (definition of the $f$ maps):] We define for each $\beta,\gamma <\alpha$ a function $f_{\beta,\gamma}$ (whose definition does not actually depend on $\alpha$:  it will be the same at every stage).  $f_{\beta,\gamma}$ is an injection from $\tau_\beta$ into $X_{\beta,\gamma}$.
...
We describe a refinement of the choice of designated supports, which we will use.  From each orbit in $\tau_\alpha$ under $\alpha$-allowable permutations choose one element, to form a set $C$.  For each element $x$ of $C$, choose its designated support $S$.  For each $y$ in the orbit of each $x \in C$, choose an $\alpha$-allowable $\pi$ such that $\pi(x)=y$, and if the designated support of $x$ is $S$, let the designated support of $y$ be $\pi[S]$.
...
\item  For each $A$, the collection $\{x \cap \tau_{-1}: x \in {\tt dom}(\pi^0_A)\}$ is pairwise disjoint  and  any litter with which the  union of $\{x \cap \tau_{-1}: x \in {\tt dom}(\pi^0_A)\}$  has large (i.e, cardinality $\kappa$)  intersection is covered by a small subcollection of $\{x \cap \tau_{-1}: x \in {\tt dom}(\pi^0_A)\}$.
...
\item[Theorem (freedom of action):]  A $\beta$-approximation $\pi^0$ will exactly approximate some $\beta$-allowable permutation $\pi$ if it satisfies the additional conditions that any domain element $x$ of $\pi^0_A$ which is a typed near-litter is $A$-flexible.
...
\item[Definition (strong support):]  We say that an ordered $\beta$-support $<_S$ with domain $S$  is strong if whenever $(x,A) \in S$ and $(x,A) D (y,B)$, we have $(y,B)\in S$, and moreover $(y,B) <_S (x,A)$ will hold in this situation.  Note that taking an unordered strong support and equipping it with the restriction of the global order on support conditions will give an ordered strong support, but a general ordered strong support might not comply with the global order where dependencies do not exist.  We require in addition that all items in its field with first component a near-litter have first component a litter.
...
\item[Definition (nice support):]  We define a condition on supports which will hold for images of strong supports under the action of allowable permutations.  We say that an ordered $\beta$-support $S$ with domain $S$ is nice iff whenever $(N,A)$ with $N$ a near-litter belongs to $S$, and $\gamma={\tt min}(A)<\beta$, and $N^\circ$ has a preimage $y$ under an appropriate $f$ map,  there is a $\gamma$-support $T$ of $y$ such that $T^{\{\beta\}} \subseteq \{c \in S:c <_S (N,A)\}$ is a support for $y$.  Further,
for any $(x,A)$ with $x$ a singleton, there is $(N,A) \in S$ with $N$ a near-litter such that $x \subseteq N$  and $(N,A) <_S (x,A)$.

\item[Observation:]  It is clear that the action of any allowable permutation sends a strong support to a nice support.  We would want to claim that any nice support is the image of a strong support under the action of an allowable permutation.  This would be managed with freedom of action technology.  One needs to enhance the given nice support with 
the designated supports (expanded to strong supports) associated with each near-litter support condition where the litter near the near-litter has a preimage under an appropriate $f$ map, then note that one needs to map each $(N,A)$ in the nice support to $(N^{\circ},A)$, which creates obligations to move atoms in $N \Delta N^{\circ}$ appropriately under $\pi_{A\cup \{-1\}}$.  The construction of
the desired map is then relatively easy, because atoms in the added designated supports are to be fixed and reshuffling of atoms in symmetric differences presents no essential difficulties, and we do not care where other atoms in the nice support are mapped. The protocol for adding items in a nice support [needed because supports are ordered in this context]  is to place them just before the place where they are needed,
then eliminate items which are duplicated (keeping the one which appears earlier): if a litter is inserted and there is already a near-litter near it in the field of the support, eliminate the later of the two items in the order (replacing it with the litter if necessary, without forgetting obligations to move atoms in the symmetric difference appropriately) and insert the atoms in the symmetric difference where appropriate, with their own designated supports as required.  Iteration of insertions
can proceed through no more than $\omega$ stages because items are inserted before things that depend on them [up to nearness of near-litters to litters], and dependency correlates with the global well-ordering on support conditions.

At the end I need to show that the action of the allowable permutation constructed sends the original nice support to a strong support (discarding the scaffolding added to assist construction of the permutation).
...
For each $g \in \tau_\gamma$, $\gamma<\alpha$, let $S^0_g$ be the strong support obtained by closing up the designated support for $g$ following the observation above on why supports can be extended to strong supports, with order derived from the global order on support conditions.
...
We define a support $S_b$ for each element of $\tau_\gamma$, $\gamma<\alpha$:  we select one element from each orbit in under $\gamma$-allowable permutations, define $S_b$ as $S^0_b$, then for each other $b'$ in the orbit choose
a $\gamma$-allowable permutation $\pi$ such that $\pi(b)=b'$ and define $S_{b'}$ as $\pi[S_b]$.  Notice that these are not strong supports, though they are close to being so (near-litters
in these supports may not be litters).

We describe the computation of an $\alpha$-support $T_b$
for $\{b\}_\alpha$ (where $b$ is an element of $B$) from the $\beta$ support $S_b$ whose construction is described above.   We compute this by a process of closing up $S_b^{\{\alpha\}}$:  the only situation we are concerned
about is items $(N,\{\gamma,\alpha\})$ in the support where $N^\circ=f_{\delta,\gamma}(d)$, $\delta<\alpha$.  In this case we insert a suitably
tagged version of $S_d$, or, in the case $\delta= -1$, the atom $\{d\}_\alpha$ and the type $\alpha$ litter containing $d$ in its $-1$-extension (which will cause no further extensions).   The resulting support will be a nice $\alpha$-support for $\{b\}_\alpha$.  This process terminates because in adding a support $S_d$ we are always adding things whose position in the designated order on their type is before the position of $d$ in its type.  Technical adjustment:  when we make insertions, we have to decide about order.  We put the inserted items in at the exact point where they are needed, omitting ones that are already present (they occur earlier) and in the case [which we must worry about because these supports are not strong]
where a near-litter occurs with small symmetric difference from a near-litter appearing earlier, replace the earlier near-litter with the union of the two near-litters, delete the later one, and insert the atoms in the symmetric difference appropriately.  Because only finitely many adjustments occur, this will converge sensibly.

It is given that $T_b$ is computed from
$S_b$, but what we actually need is something like the specification of $T_b$ being computable from the specification of $S_b$:  we cannot in fact compute the specification of $T_b$ from the cardinality of $S_b$, but we argue that there are $<\mu$ possibilities for specifications of supports $T_b$.  There are $\mu_0<\mu$ specifications for supports $S_b$ available at the current stage by the inductive hypothesis [more generally there is $\mu_0<\mu$ dominating the number of coding functions in any type below $\alpha$, since there are $<\kappa$ such types].   Any specification built the way we describe uses
...
For each $b \in B$ there is a strong support $S_b$ chosen as above, from which the support $T_b$ can be computed as described above.  If $b' \in B$ is in the range of the same coding function $\chi_{b,S_b}$ as $b$, $S_{b'}^-$ is $\pi[S_{b}^-]$ for some $\beta$-allowable $\pi$ with $\pi(b) = b'$.
If we have the further condition that $T_b$ and $T_{b'}$ are constructed from $S_b$ and $S_{b'}$ using the same sequence of insertions, and so have the same specification, it follows that there is a permutation $\pi_2$ such that $\pi_2[T_b] = T_{b'}$.  Note that $(\pi_2)_\beta[S_b]=S_{b'}$, from which it follows that $\pi^{-1} \circ (\pi_2)_\beta$ fixes $b$, since it fixes all elements of $S_b$, so $b'=\pi(b) = (\pi_2)_\beta(b)$, from which it follows
that $\pi_2(\{b\}_\alpha) = \{b'\}_\alpha$ so $\{b\}_\alpha$ and $\{b'\}_\alpha$ are in the range of the same coding function $\chi_{\{b\}_\alpha,T_b}$.  Now there are $<\mu$ possible specifications of a coding function $\chi_{b,S_b}$ followed by a specification for $T_b$ combined with exact information about the insertions by which it is constructed, so by this procedure we describe a family of $<\mu$ coding functions $\chi_{\{b\}_\alpha,T_b}$ whose range covers all type $\alpha$ singletons of elements of $B$.
