...
\item[Lemma:]  The specification of a $\beta$-support exactly determines the orbit in the action of $\beta$-allowable permutations on supports to which it belongs.
...
then $S^*_\epsilon$ is $(2,\beta,\chi_{x,T},A)$ and where $T_\epsilon=(N_\beta,A)$, $T^*_\epsilon$ is $(2,\beta,\chi_{y,T'},A),$, where $N^\circ = f_{\gamma,\beta}(y,T')$ and the coding functions $\chi_{x,T}$ and $\chi_{y,T'}$ are the same.

$T'$ is $\pi_0[T]$ for some allowable permutation, and $T_\epsilon = (N_\beta,A)$ with $N \sim \pi_0(M_\beta)\cap \tau_{-1}$, by the definition of coding functions.  [NOTE: add discussion of how the approximation is augmented -- basically, map items in $T$ to corresponding items in $T'$]
