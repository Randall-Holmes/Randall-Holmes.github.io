...
\item[supports defined:]  Where $0\leq\beta \leq \alpha$, a {\em $\beta$-presupport\/} is defined as a function from a domain $D$ with a specified well-ordering (usually $D$ is a small ordinal with the natural order, in which case we call it a $\beta$-support;  notice that any presupport can be converted to a support in a natural way)  to a collection of pairs $(x,A)$, where $A$ is a finite subset of $\lambda$ with minimum $\gamma\geq 0$ and maximum $\beta$ and $x \in \tau_\gamma$ has $x \cap \tau_{-1}$ either a singleton or a near-litter.   

Elements of the range of a (pre)support are called support conditions.

For any support condition $(x,B)$ we define $(x,B)^{\uparrow \alpha}$ as $(x,B\cup \{\alpha\})$.

We make the formal requirement on presupports (and so on supports)
...
We define the action of a $\beta$-allowable permutation $\pi$ on a $\beta$-presupport $S$:  if $S(d) = (x,A)$, $\pi[S](d) = (\pi_A(x),A)$.  In the case of $-1$-supports $S$, $\pi[\emptyset]=\emptyset$.   For supports or presupports we may write $S_d$ intead of $S(d)$.

An element $x$ of $\tau^*_\beta$ ($\beta \geq 0$) has $\beta$-(pre)support $S$ iff for every $\beta$-allowable permutation $\pi$, if $\pi[S] = S$ then $\pi(x)=x$.  We say that  an element $x$ of $\tau^*_\beta$ which has a $\beta$-support is $\beta$-symmetric. Every element of $\tau_{-1}$ has the empty set as support (by fiat).
...
It is important to note that if $S$ is a (pre)support of $x\in \tau_\beta$, $\pi[S]$ is a (pre)support of $\pi(x)$ for any $\beta$-allowable permutation $\pi$.

We will use the notation $S_\gamma$ for $S(\gamma)$ as noted above [and in those cases where $\gamma$ is not presumed to be an ordinal we reserve the same privilega].
