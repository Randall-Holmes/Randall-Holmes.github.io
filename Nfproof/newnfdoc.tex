...
We extend the notation of the previous paragraph.  We define $\sigma_B((x,C)) = ((y,C))$ iff all elements of $B$ dominate all elements of $C$  and $$\sigma((x,B \cup C)) = \sigma((y,B \cup C)).$$  $\sigma_B$ is a ${\tt min}(B)$-presubstitution, with $\sigma_{\{\beta\}} = \sigma$ and $\sigma_B$ for $B$ with more than one element
...
We indicate how to compute $\pi_1(\sigma((x,A \cup \{-1\}))$ for each $x \in L$ if we know how to compute $\pi_1(\sigma[(L_\gamma,A)])^\circ$.  If $x \in D_A \cap L$, we compute  $$\pi_1(\sigma((x,A \cup \{-1\}))= \pi_1(\sigma_0((x,A \cup \{-1\})).$$  We define $S_{A,L}$ as $\{x \in D_A: x \not\in L \wedge \pi_1(\sigma_0((x,A \cup \{-1\})) \in \pi_1(\sigma[(L_\gamma,A)]))^\circ\}$.
For each $x \in L \setminus D_A$, we compute $\sigma((x,A \cup \{-1\})$ as $$(\sigma_{L \setminus D_A,\pi_1(\sigma[(L,A)]) \setminus S_{A,L}}(x),A \cup \{-1\}).$$
...
$\sigma[( f_{\delta,\gamma}(x),A)] = (\sigma_{A \setminus \gamma}[ f_{\delta,\gamma}(x)],A)$ follows from the definition of set-subscripted substitutions.
