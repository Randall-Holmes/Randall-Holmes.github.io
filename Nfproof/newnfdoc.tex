...
We define $\tau_\alpha$ as the collection of elements $x$ of $\tau^3_\alpha$ for which there is an $\alpha$-code $(S,\Sigma)$ such that $x \cap \tau_\gamma = \chi``\Sigma$.
...
We define $\sigma^+(x)$ as $\sigma[x]$ for each $\beta$-substitution $\sigma$ and $x \in \tau_\beta$.  Note that $\sigma^+$ determines and is determined by $\sigma$.  The collection of permutations $\sigma^+$  is the collection of $\beta$-allowable permutations discussed in other treatments;
we may have some use for this below.


\item We describe the selection of designated codes for each element of our structure.

We choose a preliminary designated code for each element of the structure (this is necessary because objects other than atoms with codes clearly have more than one).
