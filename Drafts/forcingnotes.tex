...
Truth values for us are sets of conditions.  A set $A$ is a truth value iff it is closed under added information (if $a \in A$ and $b \geq a$ then $b \in A$) and regular (if for each $b \geq a$ there is $c \geq b$ which belongs to $A$, then $a \in A$).  For
any set $A$ of conditions, the closure of $A$ is the smallest truth value including $A$ as a subset.

Let $P^*$ denote the set of truth values.  For any $q \in P$ let $q^*$ be the closure of the set of all $r$ such that $r \geq q$.
