...
Let $P^*$ denote the set of truth values.
...
A {\em name\/} is a function with codomain the set of nonempty truth values, satisfying the additional technical condition that for a name $x$ and
an element $y$ of the domain of $x$ which is a function with codomain $P^*$, we must have each element of the range of $y$ being $\leq y(x)$.
...
Define $x \in_0 y$ as empty if $y$ is not a name and otherwise as the set of $q\in P$ such that there is $z\in {\tt dom}(y)$ such that $q \in y(z) \wedge z=x_{y(z)}$.  Because $y$ is a name, for all $z \in {\tt dom}(y)$, $z=z_{y(z)}$.  This defines
the pseudo-extension of $y$:  for $y$ to have a nonempty pseudo-extension, we require ``elements'' of $y$ to contain only information relevant under the truth values which put them in $y$.  The pseudo-extension is not the final extension, because it may not respect the identity relation of the target model.

Define $x =_1 y$ as $x=y$ [well, really as $P$ if $x=y$ and $\emptyset$ if $x \neq y$] if there is no $z$ such that $z \in_0 x$, and otherwise as $(\forall z:z \in_0 x \leftrightarrow z \in_0 y)$.

Define ${\tt set}(x)$ as $(\forall y:(\forall z:y=_1 z \rightarrow y \in_0x \leftrightarrow z\in_0 x))$.  A name is a name of a set if its pseudo-extension respects the identity relation of the target model.  It is important to note that the way we have defined
