...
\item[8/3/2026:]  Corrected the definition of negation which was a silly mistake.  Proofs needed to filled in to detect stupidities.

Leter, filled in outlines of some proofs.   Corrected some errors live in the meeting Monday evening (speaking from the Boise perspective).  Jakob, you are welcome to produce your own versions of this file;  please add version notes so that we can tell what version is what.
...
We note that the definition for Mac Lane is rather more complicated than the definition we will use in NFU.  But it seems important to show that there is nothing peculiarly NF-iste about the account of forcing that we give.
...
Truth values for us are sets of conditions.  A set $A$ is a truth value iff it is closed under added information (if $a \in A$ and $b \leq a$ then $b \in A$) and regular (if for each $b \geq a$ there is $c \geq b$ which belongs to $A$, then $a \in A$).  For
...
If $\phi$ and $\psi$ are truth values, $\neg \phi$ is $\{p \in P:(\forall q\geq p:q \not\in \phi)\}$.

Note that if $p \in \phi$ and $p \in \psi$, then $p \in \phi\wedge \psi$.

Note that if $p \in \phi \wedge \psi$, then $p \in \phi$ and $p \in \psi$.

We show that $\neg\neg \phi = \phi$.  $\neg\neg\phi$ is the collection of $p$ such that no $q \geq p$ is in $\neg\phi$, that is for every $q \geq p$ there is $r \geq q$ such that $r \in \phi$,
and this is equal to $\phi$ by regularity.

If we can show for every $p$ that if $p \in \phi$ then $p \in \emptyset$, it follows immediately that for any $q \in \phi$, if $q \in \phi$ then $p \in \phi$ then $p \in \emptyset$ so no $q \geq p$ belongs to $\phi$, so $p \in \neg\phi$.

If we have $p \in \phi$ and $p \in \neg \phi$ we have $p \in \emptyset$.

This actually gives a complete set of rules of deduction for $\wedge$ and $\neg$, which means we have all rules of deduction for propositional tautologies.
...
If we can show for any $x \in V$ that $p \in \phi(x)$ then by definition $p \in (\forall x:\phi(x))$.  This gives us the rule of universal generalization.

If we have $p \in (\forall x:\phi(x))$ and we have $a \in V$, then we certainly have $p \in \phi(a)$.  This gives the rule of universal instantiation.

$(\exists x:\phi(x))$ is defined as $\neg(\forall x:\neg \phi)$.

Suitable rules for the existential quantifier follow from the rules given.

All tautologies of first order logic evaluate to $P$ in the calculus of truth values.

We note that it is not at all clear that $p \in (\exists x:\phi(x))$ implies that there is a name $a$ such that $p \in \phi(a)$.  What is true is that if $p \in (\exists x:\phi(x))$ there are for each $q \geq p$, $r$ and $a_r$ such that $r \in \phi(a_r)$.

A {\em name\/} is a function with codomain the set of nonempty truth values.  I am not sure that we have to exclude the empty ones, but it seems stylistically proper to do so.
...
the pseudo-extension of $y$.  The pseudo-extension cannot be taken to be the actual extension, because it may not respect the identity relation we introduce next.

Notice for any $p$ that $p \subseteq x \in_0 y$ iff $p \subseteq  x_p \in_0 y$.  The basic reason is that if $q \in p$, $(x_p)_{q^*}=x_{q^*}$.
...
Note that $p \subseteq  x_p =_1 x$.

Define ${\tt set}(x)$ as holding iff $x$ is a name and $$(\forall y:(\forall z:y=_1 z \rightarrow y \in_0x \leftrightarrow z\in_0 x)).$$  A name is a name of a set if its pseudo-extension respects the identity relation of the target model.  It is important to note that the way we have defined
$\in_0$ ensures that these are actually bounded assertions.  These statements can be framed in the language of the target model with unbounded quantifiers, but unwinding the definitions
...
There is stuff to be inserted here.  One needs to put in the technical stuff which says that logic works correctly (that is in no way original here) [this is added above, at least in intention].  One needs
...
We want to show, where $\phi$ is a formula in which $A$ and $S$ are not bound, that $p \in (\exists S:(\forall x:x\in S \leftrightarrow x \in A \wedge \phi(x)))$.

Let $A$ be a name.  The witness we use for $S$ is the name $(x \in {\tt dom}(A)\mapsto A(x) \wedge \phi(x))$.  Note that for this to be a set the predicate $\phi(x)$ has to respect pseudo-extension in some sense.
