...
\item[Meta-Theorem:]  Every kind-bounded formula in which we have an intended assignment of kinds to parameters is equivalent to a stratified formula.

\item[Proof:]  Begin by defining a map $\tau^*$ from variables to kinds, sending each bound variable to the kind in which it is bound (we can rename variables so that no variable is bounded in two different kinds) and each parameter to its intended kind.  Of course this is a map in the metatheory:  there are no such functions in the theory itself.

Note that any formula $x=y$ must have
$\tau^*(x)=\tau^*y$ or it can simply be replaced with $x \neq x$.  Similarly, any formula
$x \in y$ must have $\tau^*(y) = \kappa(\tau^*(x))$ or is can be replaced with $x \neq x$.  Carry out any such replacements.

Express each $\tau^*(x)$ in the form $\kappa^n(\tau^*(u))$ where $n$ is chosen as large as possible such that $\kappa(u)$ is the bound of a quantifier or the intended type of a parameter in the modified formula.  Define $\tau^*(x)$
as the exponent $n$ in this form for $\tau^*(x)$.

The modified  formula is equivalent to the original formula under the hypotheses about the kinds of the parameters, and $\tau$ witnesses that it is stratified.

\item[Convention:]  The convention of {\em typed language\/} which we do not adopt routinely but do consider as a possibility, is one in which every variable $x$ in our language has an associated natural number $n$, and we suppose that each variable $x$ ranges over objects in $\kappa^{\tau(x)+1}(u)$ for some fixed object $u$.  Bound variables are understood to be restricted to the associated kind of the variable; parameters are understood to be chosen from the associated type.
