...
bound variables are systematically renamed so thet different quantified subformulas always have different binding variables.  The relation of {\em connectedness\/} between variables in a formula is defined as the smallest equivaelnce relation containing each pair of variables which appear together in an  atomic subformula of the given formula.
...
Now we describe the elimination of the exotic conjuncts, whose strange boundednesss to complements of concrete finite unions of kinds represent a stratification violation. [we write $\phi^*(x^*)$ instead of $\phi^*$ to facilitate writing many substitutions]  
...
It should be noted that all statements of equivalence here hinge on
the instances of the separation axiom which prove $\kappa^n(x) \neq \kappa(x)$ for $n>1$:  these are used to justify elimination of ill-typed formulas.
