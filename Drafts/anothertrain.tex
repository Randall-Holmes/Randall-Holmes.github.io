...
We prove in this section that every formula of our language is equivalent to a stratified formula.  We start (as above) with the stronger idea that each formula is assigned a kind as a type, in such a way that a parameter $v$ will be assigned $\kappa(v)$ and a bound variable $x$ which is bounded in a kind $\kappa(u)$ will be assigned the same type as $u$.  

Notice that if each of $u$ and $v$ is typed and $u=v$ appears in the formula and the types assigned to $u$ and $v$ are different, then $u=v$ can be replaced with $u \neq u$, since it is simply false.  If $u \in v$ appears and $u$ and $v$ are both assigned types and the type assigned to $v$ is not the kind of the type assigned to $u$, then $u \in v$ can be replaced
with $u \neq u$ as it is false.  We assume that these typing conditions hold on atomic subformulas.
...
Consider an arbitrary formula $\phi$ in which some all parameters are assigned kinds as types and all bound variables are typed (restricted to explicitly given kinds which serve as their types) or are unbounded quantifiers over stratified formulas.  Let $x$ be a variable free in $\phi$ which is not assigned a kind as type.  We show that $(\forall x:\phi)$ is equivalent to a stratified formula (mod failure to assign types to other untyped parameters in $\phi$).  Repetition of this procedure will show that all formulas are equivalent to stratified formulas.
...
which become ill-typed, and an exotic conjunct $$(\forall x^*:\bigwedge_i \kappa(x^*) \neq t_i\rightarrow \phi^*),$$ where $\phi^*$ is the result of first replacing $x$ with $x^*$, assigning it a type distinct from the $t_i$'s [any type which is a sufficiently iterated kind will give the same result], then extending the type assignment and removing ill-typed atomic subformulas, which will include  {\em all\/} of the atomic subformulas which suggested types to us, so that $\phi^*$ contains no typed free variables other than $x^*$ which are connected to $x^*$'s and is stratified with a type assignment to $x^*$ and to all variables connected to it, as well as the variables already typed.  Retain the type assignment to $x^*$ for further use though we do not bound the formula.

Now we describe the elimination of exotic components.  What is necessary is to show that if $\phi(x)$ is a stratified formula  then $$(\forall x:(\bigwedge_{i=1}^n \kappa(x) \neq \kappa(t_i))\rightarrow \phi(x)$$ is equivalent to a stratified formula.
...
We argue that this procedure can be iterated to show that all formulas are equivalent to stratified formulas (mod any given assignment of types to their parameters).  In any formula, make an assignment of types to all free variables and bounded quantifiers.  Look at an outermost unbounded quantifier  $(\forall x:\phi)$, and perform this procedure on it to obtain
an unbounded quantification $(\forall x^*:\phi^*)$ and various conjuncts with outermost typed quantifiers.  In the typed conjuncts, we can repeat the procedure on the next outermost unbounded quantifiers with extra type information, and in the unbounded conjunct we take the same approach, using the type assignment used to eliminate all free variables from $\phi^*$.  There will be no collisions between the type assignment to $x^*$ and the previous type assignments because $x^*$ is not connected to any free variable in the original formula [this procedure is carried out independently on many copies of $\phi^*$ because of the structure of the solution to quantification over complements of finite unions fo kinds, but these formulas are unconnected to each other and can be manipulated independently in parallel].  At the end of the procedure, all variables are assigned types and all atomic subformulas satisfy the typing conditions, so we can extract a stratification in the way we have described.
