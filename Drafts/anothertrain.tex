...
We argue that this procedure can be iterated to show that all formulas are equivalent to stratified formulas (mod any given assignment of types to their parameters).  In any formula, make an assignment of types to all free variables and bounded quantifiers.  Look at an outermost unbounded quantifier  $(\forall x:\phi)$, and perform the procedure described above on it to obtain
a conjunction of typed formulas $(\forall x_i \in \kappa(\tau_i): \phi(\tau_i)$ and an exotic conjunct  $$(\forall x^*:(\bigwedge_i \kappa(x^*) \neq t_i)\rightarrow \phi^*),$$ as described above.  In the typed conjuncts, we can repeat the procedure on the next outermost unbounded quantifiers with extra type information, and in the unbounded conjunct we take the same approach, using the assignment of a type to $x^*$ used to eliminate all other free variables from $\phi^*$.  There will be no collisions between the type assignment to $x^*$ and the previous type assignments because $x^*$ is not connected to any free variable in the original formula.  Application of the procedure leads to stratified formulas because types are eventually assigned to all variables in a way which respects the typing conditions, and we have described a procedure above for eliminating the exotic quantifications over complements of concrete unions of kinds once the formula quantified over is stratified.
