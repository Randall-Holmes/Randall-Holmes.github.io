...
Our theory TTGV can in general terms interpret TST  using some $\tau(x)$ as type 0, and $\tau^{i+1}(x)$ as type $i$ (replacing typed variables with variables restricted to given sets).  However, the language of our theory affords no obvious way to describe the sequence of $\tau^i(x)$'s (and in fact provably cannot describe this sequence).
...
Now we can describe a putative interpretation of the language of TSTG  in our untyped theory  TTGV.  Given a model of TTGV, we provide an atomic type notation correlated with each type $\tau(x)$ and for any type notation $u$ interpreted as $\tau(x)$, we interpret $u^+$ as $\tau^2(x)$ and $u^-$ as $\tau^0(x)$ (the type of the same type as $x$) if this is defined.
...
We describe a procedure for converting formulas of the language of TTGV into equivalent formulas which are typable (at least to a great extent).    Consider a formula $(\forall x:\phi)$ as the conjunction over all types $\tau(a)$ of formulas $(\forall x \in tau(a):\phi^*)$, where $\phi^*$ denotes the transformation of $\phi$ by the process we are describing.  Comsider existential quantifiers as expanding similarly to infinitary disjunctions.  We do not need to expand quantifiers which are bounded in types containing no bound variable.  We suppose constant denotation from the model for any variables free in $\phi$.  Now consider any atomic formula $x=y$ or $x \in y$ in $\phi^*$:  by construction, we can determine a concrete type for each of $x$, $y$, by inspection of the model element if it is free and otherwise by inspection of the bound on the quantifier over  each variable (if the quantifier binding $x$ is bounded, the fact that we have concrete denotations for free variables ensures that we have a type assignment for $x$).  Replace the formula with the False if the types of $x$ and $y$ do not permit the formula to be true.
Consider an innermost subexpression which is infinitary (an infinite conjunction or disjunction of formulas $(\forall x \in \tau(a):\phi)$, where $\phi$ is a formula in which all variables are free or restricted to types defined in terms of free variables, and so have concrete types taken from the model.  In all but finitely many of these formulas, every atomic subformula in which
$x$ appears either has $y$ wuth type $\tau^2(a)$ or $\tau^0(a)$ or is eliminable in favor of the False, and all such formulas look the same, and the outermost quantifier can be removed (if there are no occurrences of $x$ in the transformed $\phi$) or replaced with $(\forall b:\tau(b) \neq \tau(a_1) \vee \ldots \tau(b) \neq \tau(a_n):(\forall x \in \tau(b):\phi^*))$, where
the $a_i$'s are the types for which $\phi^*$ doesn't take this uniform form.  Further, this nasty formula will not have $x$ connected to any free variable in $\phi$ other than $x$ (variables assigned types depending on $a$ must have been bound), so it can be transformed into a sentence and reduced to a truth value (it reports a fact about the model).
