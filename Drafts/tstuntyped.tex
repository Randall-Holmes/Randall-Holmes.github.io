...
\item $$(\forall xyz:  (x <_\tau z \wedge y <_\tau z) \vee (z <_\tau x \wedge z <_\tau y) \rightarrow (x <_\tau y \vee x=y \vee y <_\tau x)):$$  connectedness implies trichotomy.
...
I'm reverting here for the moment to a description exploiting access to a concretely given model of TSTG in which the sets implementing the types are disjoint [any model of TSTG can easily be converted to a model with disjoint types without changing its theory as expressed in the language of TSTG].  I'll convert this back to a transformation of formulas when I have it sorted out.

First, replace the given model of TSTG with the union of countably many disjoint copies of the model, then assign the truth value False to ill-typed atomic formulas.

What we need to do is exhibit a way of taking a general formula of TTGV as an assertion about this structure and showing it to be equivalent to a typable assertion of the language of TSTG (mod the scheme which asserts disjointness of connected types and another scheme we will describe).

We describe our general procedure for eliminating quantifiers not restricted to a type.  Let $(\forall x:\phi)$ be a formula in which $\phi$ is weakly stratified (each bound variable is typable), with each bound variable [with the exception of universal quantifiers over all types mentioned in a moment] restricted to a type which is either a concrete type taken from the model or a type $\tau^i(x)$ [all integers $i$ permitted] or a type governed by a universal quantifier over all types, with the further proviso that
any subformula headed by a quantifier over all types does not contain any free variables.  Each free variable other than $x$ is to be understood to have a concrete value in the model, which of course belongs to a concrete type.

We say that two variables are connected to one another in $\phi$ iff they stand to each other in the relation which is the transitive closure of the relation of being part of the same atomic subformula.  There is a finite list of concrete types in the model which can reasonably be assigned to $x$.  If $x$ is connected to a free variable $a$, this means there is a variable $u$
with type $\tau^i(x)$ in a subformula $a=u, a \in u,u\in a$, and in each case this gives a concrete type $\tau^j(a)$ to assign to $x$ (which the reader may compute;  respectively $\tau^{2-i}(a), \tau^{3-i}(a), \tau^{1-i}(a)$).  Let $\{\tau(a_i)\}$ be the list of all the concrete types which can be attributed to $x$ in this way.
The formula $(\forall x:\phi)$ is then equivalent to $(\forall x_1 \in \tau(a_1):\phi[x_1/x]) \wedge \ldots \wedge (\forall x_n \in \tau(a_n):\phi[x_n/x]) \wedge (\forall b:(\forall x_{n+1} \in \tau(b):\phi[x_{n+1}/x]))$.  We then transform each $\phi[x_i/x]$ by converting formulas which become ill-typed to the False.  In the case of $\phi[x_{n+1}/x]$ we can
assume that $\tau(x_{n+1})$ is not any of the $\tau(a_i)$'s without imposing an explicit and unstratified restriction of $b$ to the effect that $\tau(b) \neq \tau(a_i)$ for each $i \leq n$, because we can consider what happens in the additional isomorphic copies of type $\tau(a_i)$'s; in which the same typable statements are true but the type equations are false.  In $\phi[x_{n+1}/x]$ thus transformed, no free variable
can occur which is connected to $x$, and this formula $(\forall b:(\forall x_{n+1} \in \tau(b):\phi[x_{n+1}/x])$ can further be transformed by pulling out all subformulas involving variables not connected to $x_{n+1}$ from under the scope of the quantifiers, so we really are simply looking at a typable statement.

Existential quantifiers are handled dually.

The advantage of looking at this concretely is simply that we do not have to put in conditions expressing possible equations between types $\tau(a_i)$;  we know any such equations because we assume that we know exactly what is denoted by each free variable other than $x$.  With care, this can be written as a purely syntactical maneuver, but the extra conditions which would have to be added would be annoying in not being stratified (though presumably eliminable in the end).

It is also presumably possible to avoid the maneuver of making countably many disjoint copies, which would mean that the final conjunct in our construction would be explicitly restricted in an unstratified way.  I believe I see how to eliminate the failure of stratification, but it requires writing out with extreme care, and it is better I think to see this version first.

The schemes needed to justify the equivalence of the transformed formula to the original assert that $\tau^n(x) \neq \tau(x)$ for $n>1$ and that any assertion expressible in the language of TSTG which is true of all but concretely finitely many types is actually true of all types (which we arrange by the countably many disjoint copies maneuver).

A suitable ambiguity axiom would have the same effect of allowing one to avoid restricting the quantifiers over all types by excluding the $\tau(a_i)$'s;  the manipulations of the model required to get ambiguity to hold are arguably more profound.
