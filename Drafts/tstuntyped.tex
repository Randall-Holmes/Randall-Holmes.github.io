...
any subformula headed by a quantifier over all types does not contain any free variables.  Each free variable other than $x$ is to be understood to have a concrete value in the model.

We say that two variables are connected to one another in $\phi$ iff they stand to each other in the relation which is the transitive closure of the relation of being part of the same atomic subformula.  There is a finite list of concrete types in the model which can reasonably be assigned to $x$.  If $x$ is connected to a free variable $a$, this means there is a variable
with type $\tau^i(x)$ in a subformula $a=\tau^i(x), a \in \tau^i(x), \tau^i(x)\in a$, and in each case this gives a concrete type $\tau^j(a)$ to assign to $x$ (which the reader may compute;  I'll write these in later).  Let $\{\tau(a_i)\}$ be the list of all the concrete types which can be attributed to $x$ in this way.
...
assume that $\tau(x_{n+1})$ is not any of the $\tau(a_i)$'s without imposing an explicit and unstratified restriction of $b$ to the effect that $\tau(b) \neq \tau(a_i)$ for each $i \leq n$, because we can consider what happens in the isomorphic copies of type $\tau(a_i)$'s; in which the same typable statements are true but the type equations are false.  In $\phi[x_{n+1}/x]$ thus transformed, no free variable
can occur which is connected to $x$, and this formula $(\forall b:(\forall x_{n+1} \in \tau(b):\phi[x_{n+1}/x])$ can further be transformed by pulling out all variables not connected to $x_{n+1}$ from under the scope of the quantifiers, so we really are simply looking at a typable statement.
