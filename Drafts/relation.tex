...
We provide the singleton image construction:  if $P$ is a predicate, the set $P^\iota$ exists which contains $\{\{\{x\}\},\{v,\emptyset\}\}$
exactly if $P$ contains $\{\{x\},v\}$ [we have to do a little trick here with the variable to preserve stratification].  The fact that we only use variables of special form in $P^\iota$ is irrelevant, as we can use quantification and equality to change the variables to whatever we like.
