...
\item[second version of 9/2/2026:]  This adds the proof that there are atoms in each positive indexed Type in our TSTU.
This is a consequence of the smallness of function spaces in our theory of functions in each type.  A quicker way to say this is,
the sizes of the Types are distinct inaccessibles.   A inaccessible is much larger than the power set of any smaller inaccessible, so of course almost all objects in each Type above Type 0 are atoms.  The proof given doesn't use the fact that the size of a Type is inaccessible:  the fact that each Type has strong limit cardinality is enough.

We do think that there is a way to represent this construction in NF rather than NFU (though we don't particularly recommend it).
In such a representation, the definition of smallness would be different, and $\iota``V$ and similar sets would not be small.
...
We can assume extensionality for function application:  $$(\forall fg:f=g \leftrightarrow (\forall x:f(x)=g(x))).$$ This very strong extensionality which assumes in effect that everything is a function, is harmless but might not be deemed appropriate:  one could impose the condition that ${\tt dom}(f)$ is nonempty, allowing many objects with the extension of the constant function with value the empty set.
...
We add a footnote with no obvious application to our development, but which seems important to record.  The axiom asserting the smallness of the set of functions from $A$ to $B$ for all small sets $A$ and $B$ implies that there are atoms.  We outline the proof.   A prerequisite for this argument is a discussion of Cantor's theorem in TSTU.  The usual form $|A| < |{\cal P}(A)|$ is badly typed:  the correct form (which admits the usual proof with a little tweak of typing) is
$|\iota``A|<|{\cal P}(A)|$.  The exponential map on cardinals is correctly presented as $\exp(|A|)=|B|$ iff $|\iota``B|=|{\cal P}(A)|$.  For any small set $A$, the set $A^{\chi}$ of $\chi_B$ for $B\subseteq A$ is small (because it is the set of functions from $A$ to $\{0,1\}$:  these functions are exactly the characteristic functions of subsets of $A$) and is at the same 
Type as $A$ and $\iota``(A^\chi)$ is the same size as ${\cal P}(A)$.  This means that $\exp(|A|)= |A^\chi|$.  But $\iota``V$ is small so $\exp(|\iota``V|)$ is small, but $\exp(|\iota``V|) = |{\cal P}(V)|$:  the collection of {\em sets\/} is small, and most objects in each positive type are atoms [from the TSTU standpoint:  it is clear from this very argument
that some atoms are functions with nontrivial extensions;  this argument makes it clear that our theory of functions in each type is significantly decoupled from our theory of sets which relates successive types].
