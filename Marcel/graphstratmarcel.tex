...
all unused variables get prefixed with an at sign:  this avoids not actual error but a serious risk of apparent variable capture.  It also warns you when
...
are supported (arguments can have arbitrary relative type, though defined terms and formulas must be stratified in the strong sense).   Demerits:  the notation is weird.  The order of the arguments gets reversed between the two forms of display, though this is semantically harmless.
...
set abstracts [it is also used on definition texts, which must be stratified in the strict sense], ensuring that types can be assigned to each bound
...
\item  The {\tt deft} command introduces a defined term and the {\tt deff} command introduces a defined formula.  The first argument
is a possibly primed capital letter;  the second argument is the term or formula.  These can further be accessed by assigning values to variables
in the definiendum.  I need to provide examples.
...
\item  The {\tt theoremcut} command has as its argument the name of a theorem, whose conclusion is introduced as a hypothesis in the current sequent.  This allows saved theorems to be used.
...
This creates scripts which can be used to reconstruct work done under the prover.  I close a log file by setting the log file to a default name ({\tt setlog (``done'')}).

\end{itemize}

The prover maintains a stack of sequents needing attention.  When the {\tt left()} or {\tt right()} rule is applied, one or two sequents are appended to the end of the proof [which is a list of sequents with pointers (really just lists of line numbers) from sequents to ones justifying them] and one then proceeds to the first sequent on the stack.
...
which expands $x \in y$ to $$(\forall u:u \in y \vee (\exists v:\neg(v \in x \leftrightarrow v \in u)))$$ on the left  (when the other rule does not apply).  We can report that this allows proof of Leibniz equality from coextensionality without cut.
...
(and also run again after revisions of the software.  We will try to produce and provide executable log files for each of these examples.

\subsection{Equality is transitive}

Here is a proof that equality is transitive.  Strictly speaking it is a proof of an instance of this and needs to be packaged in universal quantifiers
to be really useful.  
...
0.  ((Ax : (x e a == x e b)) -> (Ax : (a e x -> b e x)))
...
0.  (x*'? e x'! V (Ex'* : ~(x'* e a == x'* e x*'?)))

1.  (Ax'' : (x'' e x'! V (Ex'* : ~(x'* e a == x'* e x''))))
...
The entry line illustrates the new capability to use spaces, parentheses and brackets as padding in term and formula input.  There is no balance checking:
parentheses and brackets are just for the person entering the term [to make sure they do it right].

\begin{verbatim}

start ("Ay(Eu(Ax>(exy)(euy)))")
...
Fresh variable reference error
...
This is the first shakedown cruise for defined formulas in a proof.  I found bugs, and I am sure there are more.  There is a false start in this proof
which made extra work for me.

Variables from definitions which are not substituted for get moved into an inaccessible name space to avoid apparent variable capture (prefixing an at sign).
...
C = (Ax : (x e a -> x e b))
[['b', 1], ['5', 1], ['4', 0], ['a', 1], ['x', 0], ['3', 1], ['2', 0]]
start("Ax Ay Az >(&(:ax:byC :ay:bzC ) :ax:bzC)")
...
0.  (Ax : (Ay : (Az : ((C(b:y)(a:x) & C(b:z)(a:y)) -> C(b:z)(a:x)))))
...
0.  (Ay : (Az : ((C(b:y)(a:x'!) & C(b:z)(a:y)) -> C(b:z)(a:x'!))))
...
0.  (Az : ((C(b:y'!)(a:x'!) & C(b:z)(a:y'!)) -> C(b:z)(a:x'!)))
...
0.  ((C(b:y'!)(a:x'!) & C(b:z'!)(a:y'!)) -> C(b:z'!)(a:x'!))
...
0.  (x'! e {u | u e y'!} -> @x'! e {u | u e y'!})

1.  (Ax'* : (@x'! e x'* == x'! e x'*))

2.  x'! e y'!

3.  (@x'! e {u | u e y'!} -> x'! e {u | u e y'!})
----
...
\subsection{First projection theorem for the ordered pair}

Some serious debugging is still in progress, but not every bug I encountered was in the prover:  some were in my understanding of the curious
way that definitions now work.

\begin{verbatim}

deft("P","{xV=xa=xb")
P = {x | (x = a V x = b)}
[['b', -1], ['10', -1], ['a', -1], ['8', -1], ['9', -1], ['7', -1], ['x', -1], ['6', 0]]
deft("O",":a  :ax:bxP  :b :ax:byP P")
O = P(b:P(b:y)(a:x))(a:P(b:x)(a:x))
[['x', -2], ['16', -2], ['17', -2], ['y', -2], ['13', -2], ['14', -2], ['12', -1], ['15', -1], ['11', 0]]
deft("'P","{xAy>eyaexy")
P' = {x | (Ay : (y e a -> x e y))}
[['a', 1], ['24', 1], ['23', 0], ['y', 0], ['26', 0], ['25', -1], ['x', -1], ['22', 0]]

\end{verbatim}

Above are the definitions of unordered pair, ordered pair and a definition of the singleton of the first projection of an ordered pair.

In the following theorem we prove that the last function really does what we just said it does.

\begin{verbatim}

start("AxAy =:a :xx:yyO 'P {u=ux")
...
0.  {@x | (A@y : (@y e O(y:y'!)(x:x'!) -> @x e @y))} = {u | u = x'!}
...
0.  (Ax* : (x* e {@x | (A@y : (@y e O(y:y'!)(x:x'!) -> @x e @y))} == x* e {u | u = x'!}))
...
0.  (x''! e {@x | (A@y : (@y e O(y:y'!)(x:x'!) -> @x e @y))} == x''! e {u | u = x'!})
...
0.  x''! e {@x | (A@y : (@y e O(y:y'!)(x:x'!) -> @x e @y))}
----
...
0.  (@y? e O(y:y'!)(x:x'!) -> x''! e @y?)

1.  (A@y : (@y? e O(y:y'!)(x:x'!) -> x''! e @y?))
----
...
0.  @y? e P(b:P(b:y'!)(a:x'!))(a:P(b:x'!)(a:x'!))
...
0.  @y? e {@x | (@x = P(b:x'!)(a:x'!) V @x = P(b:y'!)(a:x'!))}
...
1.  (y*? e O(y:y'!)(x:x'!) -> x''! e y*?)

2.  (A@y : (@y? e O(y:y'!)(x:x'!) -> x''! e @y?))
----
...
3.  (y''! e {u | x''! e u} -> {@x | (@x = x'! V @x = x'!)} e {u | x''! e u})
----
...
2.  (y''! e {u | x''! e u} -> {@x | (@x = x'! V @x = x'!)} e {u | x''! e u})
----
...
1.  (y''! e {u | x''! e u} -> {@x | (@x = x'! V @x = x'!)} e {u | x''! e u})
