...
$I(a:t)$ ({\tt :atI} in the input language) [$I(a:x)$ actually works correctly though it displays as $\{x:x=x\}$ at the moment:  the variable capture is only apparent (the prover knows all about it) it just isnt seen in the display [which I do need to fix, but it is in a way fun].  If we define {\tt C} as $(\forall x:x \in a rightarrow b \in a)$ then we can represent $t \subseteq u$
...
I advise the reader that stratification data is now more crowded.  Installing the definition feature required that I create graphs for terms as well as formulas, and graphs for terms need a base node for the entire term.  So all subterms acquire base nodes, and there are a lot of additional nodes in the graphs.  I should clean the base nodes out of stratification display output, but I cannot remove them from graphs;  they are essential to making the definition feature work.
