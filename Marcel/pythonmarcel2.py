...
# Python version of Marcel -- 9/19/2021

import os

def versiondate():
    print('\n   Version of 9/23/2021 7 PM  \n   \
    eliminating quantifiers in all rules, stronger instantiation \n \
...
# 9/29/2021  This version seems to work as I would want.  It would
# be nice to be able to toggle to the old behavior -- or would it?  Maybe this
# is just as useful.  It would be good to compact indices of constants
# and instantiables.

# the intention here is to cause left rules for the universal quantifier
# and right rules for the existential quantifier to eliminate the quantifiers;
# this is enabled by changing how instantiation works.

# 9/23 the proliferation of propositions is stopped.

# it appears to work (9/19/2021 7 pm).  I can't see how to get listsub2
# to handle genealogy sensibly.

#  No, it doesnt work for nested quantifiers.  Instantiate needs to act strongly on
# variables and instantiables of higher index.

# Now it appears to work for nested quantifiers.  Strategy is required
# to avoid vast proliferation of terms.  There may be ways to avoid
# some of the term copying.
...
def substitute(t,v,T):
     ...
     if t[0] == 'instantiable' and v[0]=='instantiable' and int(t[2])>int(v[2]) and (T[0]=='instantiable' or T[0] == 'constant')\
        and int(T[2]) > int(v[2]):
         #print (v[0]+v[1]+v[2])
         #print (T[0]+T[1]+T[2])
         return [T[0],T[1],str(int(T[2])+nextobjectindex2)]
    ...
    if P[0]=='unary' and P[1]==['operator','A'] and P[2][0] == 'binary' \
       and P[2][1]==['operator',':'] and P[2][2][0] == 'variable':
         ...
         return [[[fix(substitute(['instantiable',P[2][2][1],\
                  ...
                  #omitted fix(P) final item to list
       ...
       and P[2][2][0]=='binary' and not(type(P[2][2][2])=='prop') and not(type(P[2][2][3])=='prop'):
            ...
            if P[2][2][2][0]=='variable':
                return [[[fix(substitute(['instantiable',P[2][2][2][1],str(nextobjectindex)],P[2][2][2],\
                  ['binary',['operator','->'],P[2][2],P[2][3]]))],[],'']]  #omitted final fix(P)
 ...
 #       nextobjectindex=nextobjectindex+1
    ...
    if P[0]=='unary' and P[1]==['operator','ER'] and P[2][0]=='binary'\
       ...
       and P[2][2][0]=='binary' and not(type(P[2][2][2])=='prop') and not(type(P[2][2][3])=='prop'):
            ...
            if P[2][2][2][0]=='variable':
                return [[[],[fix(substitute(['instantiable',P[2][2][2][1],str(nextobjectindex)],P[2][2][2],\
                  ['binary',['operator','&'],P[2][2],P[2][3]]))],'']]  #skipped final fix(P)
...
# this is for replacing an instantiable with two different items,
# when it occurs (without echoing terms)

def listsub2(t1,t2,v,L):
    if L==[]:  return []
    global theproof
    global nextobjectindex
    global nextobjectindex2
#    return [[substitute(t,v,L[0][0]),L[0][1]]]+listsub(t,v,L[1:])
    ...
    list2 = []
    while(not(list1==[])):
        S=substitute(t1,v,list1[0][0])
        T=substitute(t2,v,list1[0][0])
        if not (S==T or S==list1[0][0]): list2 = list2+[[S,list1[0][1]],[T,list1[0][1]]]
        if S==list1[0][0]: list2 = list2+[[S,list1[0][1]]]
        if S==T and not(S==list1[0][0]): list2 = list2+[[T,list1[0][1]]]
        list1 = list1[1:]
    return list2
...
def sequentsub2(t1,t2,v,S):
    global theproof
    return transformsequent([S[0],listsub2(t1,t2,v,S[1]),listsub2(t1,t2,v,S[2])])
...
def proofsub2(t1,t2,v,P):
    global theproof
    global nextobjectindex
    global nextobjectindex2
    if P[1]=='goal':  return [sequentsub2(t1,t2,v,P[0]),'goal']
    return [sequentsub2(t1,t2,v,P[0]),prooflistsub2(t1,t2,v,P[1])]

def prooflistsub2(t1,t2,v,L):
    global theproof
    global nextobjectindex
    global nextobjectindex2   
    if L==[]:  return []
#    return [proofsub(t,v,L[0])]+prooflistsub(t,v,L[1:])
    ...
    list2 = []
    while(not(list1==[])):
        list2=list2+[proofsub2(t1,t2,v,list1[0])]
        list1=list1[1:]
    return list2
...
def instantiate(t,v,P):
    ...
    global nextobjectindex
    global nextobjectindex2
    nextobjectindex2=nextobjectindex
    if not v[0]=='instantiable':  return P
#    if v[0] == 'instantiable' and t[0] == 'instantiable':
#        printerror('do not instantiate with an instantiable')
#        return P
    if type(t)=='class':
    ...
    if not maxindex(t)<maxindex(v):
        ...
        return P
    n=nextobjectindex2
    return proofsub2(t,["instantiable",v[1],str(int(n))],v,P)
