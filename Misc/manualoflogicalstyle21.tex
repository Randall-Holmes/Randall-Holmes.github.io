...
\title{Manual of Logical Style (fresh version 2021)}
...
$(\forall x \in {\mathbb R}:(\forall y \in {\mathbb R}: x+y=y+x))$.  The order of the quantifiers doesn't matter:$(\forall y \in {\mathbb R}:(\forall x \in {\mathbb R}: x+y=y+x))$ says the same thing.  This can be abbreviated $(\forall x,y:x+y=y+x)$ and similarly we could write $(\forall x,y,z:(xy)z=x(yz))$ to express the associative law of multiplication.  Multiple existential quantifiers are similarly indifferent to order and can be abbreviated similarly.
...
I'm not planning to assign formal proofs using these rules, but I will use something like this format in proofs that I do on the board.  When you write proofs in English, you should have these thought patterns in your head.  And you do already.  I'll illustrate this by recasting the proof that the sum of two odd numbers is even in a form with explicit quantifiers, and you should see that the pattern of reasoning is the same one you learned in section 5.
\newpage
\begin{description}
...
I do not suggest that you write proofs in this exhausting way!  Certainly the way that the goal echoes at the end of the rule of witness introduction does not need to be reflected in your proofs in English.  The purpose of presenting the proof in this way is to let you see that an argument you already did before we explicitly talked about quantifiers actually used three of the four quantifier strategies, and fairly explicitly!
