...
\subsection{Additional remark on two line notation and dashes}

Two-line notation may be used for the graph of any relation.  On the top row there may be domain elements;  on the bottom row codomain elements.  If writing notation for a graph, there is no reason to use dashes.

Two line notation for a relation or function has an additional feature:  one may put a dash in the top row and put an element of the codomain which is not in the range below it.  In notation for a relation which is not a function, you may put a dash in the bottom row and put an element of the domain which does not relate to anything in the codomain in the top row above it.  The intention is that the notation for a relation or function as opposed to its graph must have all domain elements in the top row and all codomain elements in the bottom row, for full information, and the dashes are a device to tell us about domain or codomain elements which are actually not used.

Levin introduces this notation for functions only, and requires that domain elements be written on the top row in order.  I'll tell you in instructions whether I want you to do that.  I allow the notation for general relations, so you might have a domain element appear in two different columns mapping to different things (which is a signal that the relation is not a function!)

In general, I will expect you to be able to read two line notations with dashes in them, but not to write them:  it will be sufficient for you to write two line notations for the graph of a relation (uaually a function).
