...
s "[data:?x@state:?1]";
assign "?x" "[data:?x@state:?1]";
rri "ARROWRETRACT"; ex();
wb();
p "REXPRETRACT";

defineconstanttype "REXPRETRACT" "rexp:?x" "[data:?x@state:?1]";
...
s "rexp:var@?u";
ri "REXP00"; ex();
downtoleft "var@?x"; ri "var"; ex();
up(); ri "EVAL"; ex();
ri "LEFT@TYPES**STATETYPE"; ri "EVAL"; ex();
top(); right(); ri "TYPES"; ex();
ri "BIND@address:?u"; ri "LEFT@ $STATETYPE"; ex();
up(); rri "var"; ex();
wb(); 
...
defineinfix "EXPPLUS" "(?x!+!?y)" "[(Nat:(rexp:?x)@?1)+Nat:(rexp:?y)@?1]";
	...
	(* new version is extensional -- this is a model for any such definition *)
...
s "rexp:?x!+!?y";
ri "REXP00**EVERYWHERE2@EXPPLUS"; ex();
ri "EVERYWHERE2@EVAL"; ex();
ri "EVERYWHERE2@REXP0"; ex();
ri "EVERYWHERE2@TYPES"; ex();
downtoleft "?x+?y";
ri "TADDTOP@PLUSTYPE2"; ex();
uptols "NATDATA"; ri "NATDATA"; ex();
...
ri "EVERYWHERE2@REFLEX"; ri "TREMTOP@REXP0"; ex();
up(); up(); rri "CRULE3"; ex();
