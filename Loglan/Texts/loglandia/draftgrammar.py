...
L("QuoteCmapuaUnit <- (&caprule [Ll] [Ii] juncture? [Uu] juncture? ', '? [ ]* (CVV/CCV/CVC) !([ ]* Word))")
...
L("phoneticutterance1 <- (NameWord/([ ]* LiQuote)/([ ]* QuoteCmapuaUnit)/([ ]* NameWord)/([ ]* AlienWord)/([ ]* Cmapua)/([ ]* '--')/([ ]* '...')/([ ]* Borrowing ![y])/([ ]* Complex))+")
...
L("LIU1 <- (([ ]* LIU0 !V1 comma2? (Word/PreName))/([ ]* QuoteCmapuaUnit)/([ ]* (L II TAI)))")
...
L("UI0 <- (&Cmapua (UA/UE/UI/UO/UU/OA/OE/OI/OU/OO/IA/II/IO/IU/EA/EE/EI/EO/EU/AA/AE/AI/AO/AU/(B EA)/(B UO)/(C EA)/(C IA)/(C OA)/(D OU)/(F AE)/(F AO)/(F EU)/(G EA)/(K UO)/(K UU)/(R EA)/(N AO)/(N IE)/(P AE)/(P IU)/(S AA)/(S UI)/(T AA)/(T OE)/(V OI)/(Z OU)/(L OI)/(L OA)/(S IA)/(S II)/(T OE)/(S IU)/(C AO)/(C EU)/(S IE)/(S EU)))")
