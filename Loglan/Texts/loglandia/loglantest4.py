...
L("__LWbreak <- (!(Oddvowel) !connective)")
...
L("A <- (__LWinit !(TAI0) (((N [u]) &((u / (N [o])))))? ((N [o]))? A0 (NOI)? !((([ ])+ PANOPAUSES PAUSE)) !((PANOPAUSES !PAUSE [, ])) ((PANOPAUSES ((F i) / &PAUSE)))? !(Oddvowel))")
...
L("CA1 <- ((((N u) &(((C u) / (N o)))))? ((N o))? CA0 !((([ ])+ PANOPAUSES PAUSE)) !((PANOPAUSES !PAUSE [, ])) ((PANOPAUSES ((F i) / &PAUSE)))? !(Oddvowel))")
...
L("I <- (__LWinit !(TAI0) i !((([ ])+ PANOPAUSES PAUSE)) !((PANOPAUSES !PAUSE [, ])) ((PANOPAUSES ((F i) / &PAUSE)))? __LWbreak)")
...
L("DIO <- (__LWinit ((B EU) / (C AU) / (D IO) / (F OA) / (K AO) / (J UI) / (N EU) / (P OU) / (G OA) / (S AU) / (V EU) / (Z UA) / (Z UE) / (Z UI) / (Z UO) / (Z UU)) __LWbreak)")
...
L("PREDA <- (([ ])* &(caprule) (Predicate / LWPREDA /\
!([ ]) NI RA)\
!connective)")
...
L("predunit1 <- ((SUE / (NU (freemod)? GE (freemod)? despredE (((freemod)? geu (comma)?))?) /\
(NU (freemod)? PREDA) / ((comma)? GE (freemod)? descpred (((freemod)? geu (comma)?))?) \
/ abstractpred / (ME (freemod)? argument1 (gap)?) / PREDA)) (freemod)?")
...
L("descriptn <- (!(LANAME) ((LAU wordset1) / (LOU wordset2) / \
(LE (freemod)? ((((!(mex) arg1a (freemod)?))? ((PA2 (freemod)?))?))? mex (freemod)? descpred) /\
(LE (freemod)? ((((!(mex) arg1a (freemod)?))? ((PA2 (freemod)?))?))? mex (freemod)? arg1a) / \
(GE (freemod)? mex (freemod)? descpred) / (LE (freemod)? ((((!(mex) arg1a (freemod)?))? ((PA2 (freemod)?))?))? descpred)))")
...
L("arg1 <- (abstractn / (LIO (freemod)? descpred (gap)?) / (LIO (freemod)? term (gap)?) / (LIO (freemod)? mex (gap)?) / (LIO stringnospaces) / LAO / LANAME /\
(descriptn  (((((comma)? CI (comma)?) / (comma &(nonamemarkers) !(AcronymicName))) name))? (gap)?) / LANAME2 / LIU1 / LIE / LI)")
...
L("freemod <- ((NOUI / (SOI (freemod)? descpred (gap)?) / \
DIE / (NO1 DIE) / (KIE (comma)? utterance0 (comma)? KIU) / \
(([ ])* HUE0 (CANCELPAUSE / (([ ])* &(C1))) name (gap)?) / \
(HUE !(badspaces) (freemod)? descpred (((((comma)? CI (comma)?) / (comma &(nonamemarkers) !(AcronymicName))) name))? (gap)?) / (HUE !(badspaces) (freemod)? statement (gap)?) /\
(HUE !(badspaces) (freemod)? termset1) / (([ ])* HUE0 ((',' ([ ])+) / (([ ])* &(V1))) name (gap)?) / (HUE stringnospacesclosedblock) / voc / CANCELPAUSE / comma !Name / JO / UI1 / (([ ])* '...' ((([ ])* &(letter)))?) / (([ ])* '--' ((([ ])* &(letter)))?)) (freemod)?)")
