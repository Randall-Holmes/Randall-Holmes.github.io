...
\date{1/20/2022, 11 pm Boise time}
...
The use of the marker {\bf gio} to separate the subject from following untagged arguments before the predicate is now optional in a test parser.  I still prefer it, but I'm allowing this to be considered by the community.
...
\item[There is no pause/{\bf gu} equivalence:]  There is no use of pauses as right closers.  There was a quite differently motivated change in behavior of argument lists which has effects which might remind one of pause/{\bf gu} equivalence, now shelved.
...
\footnote{THIS HAS BEEN SHELVED (at least for the moment):  In my favorite version of the parser, trailing arguments will be read as subjects of following sentences by preference.  This produces some effects which might remind one of pause/{\bf gu} equivalence.  A new device,
...
A phonetic hyphen {\bf n} appears only when followed by {\bf r} in the next djifoa.  The phonetic hyphen {\bf r} may be replaced by {\bf rr} (a continuant in its own syllable) if it follows a mandatory monosyllable ending in {\bf i} (English speakers at least find the original form hard to pronounce).  A phonetic hyphen is never final in a complex predicate nor will it follow a five-letter djifoa.  A phonetic hyphen {\bf y} is always unstressed, and appears by itself in a syllable or in a C{\bf y} syllable in the context CV-C{\bf y} of a CVC djifoa (CVC-{\bf y} also being allowed;  the {\bf y} in a five-letter djifoa in non-final position may participate in a C{\bf y} or cc{\bf y} syllable, which is also always unstressed).  A borrowing djifoa is always preceded by {\bf y} if not initial (the {\bf y} will be a hyphen or a constituent of a five-letter or borrowing djifoa) and includes an appended {\bf y} if not final.
...
\item[ci:]  A general ``verbal hyphen".  Because it can be used between items in serial names, it is a name marker.   When it is not followed by a pause (either explicitly marked or required by phonetics) or immediately by a name word, it is not a name marker (this protects us from very strange and hard to track parse failures related to the other entirely different uses of {\bf ci}).
...
\begin{description}
\item [ci:]  The main use is very tightly binding predicate modification.  This syllable has other uses binding items together, notably numeral indices to pronouns.
Note that if one pauses after it, it becomes a name marker, with weird phonetic consequences.
...
\item[gi:]  separates fronted arguments or modifiers from following parts of a sentence.  The variant {\bf goi} is used for head quantifiers.
...
\footnote{ {\bf alternative parser approach shelved:}  In an alternative version of the parser, there is a further optional component of this class of sentence (the basic SVO statement) which is a new
...
\footnote{(shelved) In the alternative parser, the further restriction is placed on the classes
...
{\bf I no longer propose adoption of the ``alternative parser" approach, but I preserve this reference to it.}  In this section, we describe the grammar changes involved in adoption of this further proposal in one place.
