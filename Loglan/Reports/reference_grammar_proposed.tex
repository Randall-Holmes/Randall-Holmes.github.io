...
\begin{description}
\item[10/11/2025:]  Added permission to link final {\em go} phrases in sentence predicates and termsets with forethought and afterthought connectives.  Made the language in the footnote describing impermissible medial consonant pairs clearer (and longer).
\item[7/20/2025:]  Added the remark that APA and IPA words must be closed with {\bf -fi} when followed by a vowel initial word to all relevant discussions in the text.  This remark did occur in one place, but it needed to appear in some other places.  I do remark here that all remarks about APA and IPA words of a phonetic nature apply to AKOU and IKOU words:  the only differences between these classes are semantic.
...
\item[One must pause after IPA, APA:]  One must pause explicitly (comma written) after these connectives or take the new approach of closing them with {\bf -fi}.  If the following word is vowel-initial, one must close the word with {\bf -fi}, because a comma pause before a vowel initial word is {\em not\/} an explicit pause.
...
\item The third part, which is optional, consists of one or two final consonants.  There is a list of impermissible medial pairs and a list of impermissible medial triples.\footnote{This footnote is quite long and continues on the bottom of the next page! The impermissible medial pairs consist of 

\begin{enumerate}

\item all doubled consonants, 

\item  any pair beginning with {\bf h}, 

\item any pair both of which are taken from one of the following sets: {\bf cjsz}, {\bf fv}, {\bf kg}, {\bf pb}, {\bf td} [this rules out voiced and unvoiced versions of the same consonant as a medial pair, and slightly more],
\item any of {\bf fkpt} followed by either of {\bf jz}, \item {\bf bj}, \item and {\bf sb}.\end{enumerate}
...
\item[tau:]  ``even'', as in ``even I know that'', ``surprisingly'', ``unexpectedly''.
...
to indicate stress in writing.\footnote{The word {\bf kia} is listed as having the effect of cancelling the previous word.  It is now implemented.  The units it eliminates are not precisely words, and at present it does not work very well.  An essay is owed to the gentle reader.}
...
We generalize this pause rule in a way which has taken time to settle down.  Whenever a name marker is used, it must be followed by a block of text not containing whitespace which is followed by an explicit pause.  This is not intended to constrain where one may reasonably pause.  \newline {\bf La Blabi--Lepsu, ga fundi la Djan} is the most natural way to articulate this phrase, arguably, and the use of the interverbal hyphen allows one to form longer uninterrupted blocks of text to be articulated without pause.  Merely omitting whitespace or using syllable breaks often also works, but it does not work for whitespace after a predicate, which signals placement of a stress, so cannot be omitted or replaced with a hyphen as a rule.
...
A change in the grammar (NEW) as of 10/11/2025 permits the final structures {\bf go} + {\tt barepred} in a sentence predicate to be linked by forethought and afterthought connectives.  It is important to note that final structures starting with {\bf go} in descriptive predicates cannot be so linked.  {\bf La Meris, penso go slano la Djan, e go gudbi la Tim}:  Mary thought more slowly than John, and better than Tim.
...
A grammar modification (NEW!) on 10/11/2025 allows multiple final  {\bf go} + {\tt barepred} items in a termset, linked by forethought and/or afterthought logical connectives.
