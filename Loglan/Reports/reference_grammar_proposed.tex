...
\date{2/12/2022, 1 am Boise time}
...
A note about the pronunciation of {\bf e}:  both Loglan 1 and Loglan 3 suggest a different pronunciation of {\bf e} before vowels, which Brown says is the sound in English {\em ei\/}ght.  I do not know exactly what he meant.  If this is a pure sound, it is acceptable.  It cannot be the diphthing {\bf ei}, because {\bf lea} and {\bf leia} are distinct words, for example.
...
My efforts since 2013 have mostly been aimed at being able to parse the language unambiguously at phonetic, lexical, and grammatical levels.  I have commented as I have described this on the meaning of the constructions, of course.  In this section I will focus on semantics, stating general principles.

Loglan is not standard first order logic notation (though it is among other things a first order logic notation).  I state a couple of important propositions about this.

The basic sentence of logical notation is of the form $P(x_1,...,x_n)$, where $P$ is the predicate and the $x_i$'s are arguments (constants or variables) in which constants have single reference.

For good or ill, this is not the form of the Loglan sentence, which is better written $(x_1 P (x_2,...,x_n))$.  The first argument is different and lives at a different level of the sentence:  I refer to it grammatically as the subject.  The sutori (second and subsequent) arguments live in a list, which is
not at the same level of the sentence:  they are part of the predicate.  The logic of the language also involves a special role for the first argument in many ways.

Terms (clauses introduced with {\em pa} or {\em kou} connectives) are best thought of as additional arguments added to the sentence, in an additional place dictated by the ``preposition".    The effect of case tags is similar to that of prepositions.   Arguments prefixed with {\em goi} are to be thought of as simply additional arguments we mention in
connection with the sentence without stipulating their connection (if any) to it.   And that is actually all we need to get the effect of prenex strings, as we will discuss below.

Logical connectives {\bf e}, {\bf a} and their kin, and the negation operator, can link sentences.  In first order logic, they {\bf only} link sentences.  In Loglan they can link all sort of subconstructions of the sentence.  Such constructions are entirely avoided in standard first order logic, and they prefigure
the essential logical mechanism which Loglan (and natural languages) have which is carefully avoided in formal logical notation, which is {\em multiple reference\/}, though it should be noticed that logical connectives can link predicates and generally substructures other than arguments, in the ways that the structure
of the grammar permits.  The general rule for expanding constructions with logical connectives (we write a general connective as $\circ$) is that if $P$ is a sentence with an unspecified component $Q$ (write $P[Q]$ for this and $Q$ has the form $P[Q_1 \circ Q_2]$, the meaning of this is $P[Q_1] \circ P[Q_2]$ (here we abstract from the fact
that the connective may have a different form when connecting substructures).  Similarly, in those contexts where {\bf no} represents logical negation, $P[{\bf no} Q]$ simply means $\neg P[Q]$.  This is vitally important:  there is no option which allows Rice's treatment of negative arguments and preserves the transformation structure of Loglan described here.  This expansion can only be applied if no indefinite argument appears before $Q_1 \circ Q_2$, resp.  ${\bf no}Q$, or if logical properties of those earlier indefinites permit it (awareness of logical properties of connectives, quantifiers and other multiple reference forms is essential to
understanding Loglan, or indeed to understanding formal speech in natural languages).  Another crucial point is that the order is different where a predicate is linked:  a logically connected predicate is expanded {\em after\/} the subsequent arguments are expanded.  This issue doesn't arise with quantifiers because we do not for the most part bind predicates.



So {\bf La Djan, e la Alis, cluva le bakso, a la hasfa} says that John and Alice each love either the box or the house (a different one of these perhaps in each case) because the {\em e} must be expanded first, and {\bf Le bakso, a la hasfa nu cluva la Djan, e la Alis} says that one of the box or the house is loved by both of them.

{\bf Mi cluva ca tsodi la Djan, e la Tim\/} says that I love or hate John, and I love or hate Tim (possibly different in each case).  So in this respect the language behaves weirdly as if it were SOV...   This is most important in understanding how negations on the predicate are expanded:  a negation on the predicate cannot be exported to the head of the sentence until all arguments have singular reference.  This exemplifies the difference in order where predicates are concerned.

The full notion of order is as follows:  what is in the subject is expanded first, then what is in the object(s) in order, then what is in the predicate.  If one passes to sub-sentences, this order acts within them.  The head of a subordinate clause with {\bf jio} (not with {\bf ji})  is taken to occur at the position of the first mention of the head in the subordnate clause.

An example I am musing about is {\bf Mi donsu je ra le bakso su mrenu} versus {\bf Mi donsu ra le bakso su mrenu}.  The second is easy:  for each man there may be a different box.  But I think that in the first, we must expand the object first, so in fact there is a single man (or a number of common recipients) to whom we give all the boxes.

This is the simplest introductory example of the mechanism of multiple reference in Loglan.  There is an exception in subordinate clauses to the order rule, or rather a modification of where we understand a term to appear first.  An argument with a subordinate clause is understood to appear first where it is first referenced inside the clause.
So {\bf Da ji la Djan, e la Alis, gui jio  la Tim, a la Ailin, cluva da gui ga gudbi} [though I am not sure I should allow reference with {\em da} to carry the freight of an indefinite this way]  says that John and Alice (the John, respectively Alice,  who  is loved by Tim or Eileen, the same one in each case) are good.  {\bf la Tim, a la Alis} is expanded first, because it appears before the first appearance of {\bf da} in the subordinate clause.   I know how I want this to work, but precise mechanisms for transforming subordinate clauses to clarify their meaning are yet to be stated.  {\bf Da ji la Djan, e La Alis, gui jio  da nu cluva la Tim, a la Ailin, gui ga gudbi}  allows each of John and Alice to love a different one of Tim and Eileen.
This principle is enunciated above in connection with quantified arguments, but such principles are intended to be general.  This is needed, because the subordinate clause appears rigidly {\em after} its head, and we may want to say things in which arguments first mentioned inside the subordinate clause are to be expanded first.

Just as we see in quantification, the scope in which logical expressons in {\em lepo} clauses and similar constructions are expanded is the sentence in the {\em lepo} clause:  {\bf Mi djano lepo le bakso, a le hasfa ga blanu}  says {\bf Mi djano lepo le bakso, ga blanu, ica le hasfa ga blanu}, not {\bf Mi djano lepo le bakso ga blanu, ica mi djano lepo le bakso ga blanu}.  In our view,
{\bf Ra ba cluva be} and {\bf Be cluva raba} are the primary forms:  the principal (and in fact the {\em only}) effect of moving things into prenex is to control the order in which arguments are mentioned.  The semantics of the prenex string is exactly described above:  the arguments in a prenex string with {\bf goi} are simply mentioned.

In formal logical notation, quantifiers are expressed in prenex form in the first instance.  $(\forall x:\exists y:Lxy)$ and $(\exists y:\forall x:Lxy)$ might express ``Everyone loves someone" and ``Someone is loved by everyone".  We can make these statements in this form:  {\bf Ra ba be goi ba cluva be}, {\bf Be ra ba goi ba cluva be}.
Particularly given the history of the language, one might think that these were the primary forms:  that the more typical {\bf Ra ba cluva be} and {\bf Be nu cluva ra ba} are to be understood as having the prenex forms as their deep structure.  This is not our view.

An argument like {\bf Ra mrenu} ({\bf Ra ba} is similar to the logically connected arguments we started with {\bf Ra meba})  sets {\bf mei} as a bound variable subsequently and takes the (possibly infinite or very large) conjunction (referring to ``and") of sentences in which particular objects which are {\bf mrenu} replace the initial {\bf Ra mrenu} and all occurrences of {\bf mei} that it binds.
{\bf Su mrenu} expands similarly as a disjunction (referring to ``or").  The numerical quantifiers are similarly handled but more complicated.
Where more than one such argument appears in a sentence, expand the one that occurs earlier first, just as with the logically connected sentences (noting the same technicality about where the head of a subordinate clause is taken to appear first).  An argument {\bf Ra ba} behaves similarly, with the underlying predicate being {\bf me ba} and the bound variable being {\bf ba}.

This scope convention explains the difference between {\bf Ra ba be goi ba cluva be} and {\bf Be ra ba goi ba cluva be} without ascribing any semantics to the prenex string other than what I have stated above:  the only effect of the prenex string is that it gives the ability to control the order in which arguments are first mentioned with precision.  That is actually pretty
much what the for all and there exists phrases in English do.

There is additional ability to control the scope of a quantifier which doesn't exist, or is hard to effect, with logically connected arguments:  if the bound variable {\bf mei} introduced by {\bf ra mrenu} occurs outside the scope of a {\em lepo} clause in which {\bf ra mrenu} occurs, this expands the scope of the quantifier (and can expand the scope of previously introduced quantifiers).
{\bf Mi djano lepo ba mormao la Djan guo, ice mi nia bleka ba}  says that I believe that someone killed John, and moreover I am looking at him (which means the scope of the quantifier is the entire sentence).  ``There is $x$ such that I believe that $x$ killed John, and I am looking at $x$" is the expanded form.  {\bf Mi djano lepo ba mormao la Djan} says
``I believe that there is $x$ such that $x$ killed John", which does not say that I believe that a specific person identified by me killed John.  More pointedly, {\bf Mi djano lepo ba mormao la Djan guo, peu ba} says that not only do I believe that there is someone who killed John, but there is somone about whom I believe that they killed John, all without saying anything about that person except the vague {\bf peu ba} which moves the scope.  I reiterate that everything we do with prenex quantifiers is just a special case of the general mechanisms of scope control:  the semantics should be the same, but the prenex forms are not in my mind primary, or simpler.

I think that quantifier scope does run across ICA connectives.  I think a useful convention would be that application of a quantifier to a variable which has just been in use opens a new scope with that bound variable, in which that variable will be used only in the new way. This, and use of disjoint variable sets, should make it possible to link quantified sentences with ICA connectives without unwanted interactions between their variables.

An argument {\bf Le mrenu} has plural reference:  it refers to many men and asserts what is said of each of them.  But it is not the same as {\bf ra (me) le mrenu}.  {\bf Le mrenu} is not a variable binding construction.  A reference {\bf mei} to it is a multiple with the same extension.  So {\bf le mrenu ga cluva mei} means ``Each of the men I have in mind loves each of the men I  have in mind", whereas {\bf Ra le mrenu ga cluva mei} means ``Each of the men I have in mind loves himself", because {\bf mei} is bound in this case.  Apart from the difference in anaphora, issues similar to those which arise with quantification can occur.  {\bf Le mrenu ga no fundi le fumna} is not the same as {\bf No gu. le mrenu ga fundi le fumna}:  the first says
that each of the men I have in mind dislikes each of the women I have in mind, and the second says it is not the case that all of the men I have in mind love all of the women I have in mind (in fact the same things it would say if I prefixed things with {\bf ra});  the {\bf no} cannot be exported to the head of the sentence because the arguments (both of them!) occur ``before"
the two arguments (recalling that the predicate is expanded after the subject and the object(s)).

I have recently introduced a special NU word {\bf kue} (which can be indexed {\bf kuete, kueto} to handle further places) which copies a multiple:  {\bf Le mrenu ga kue cluva} expands to {\bf Le mrenu ga cluva le mrenu}, all the men love each other and themselves.  In this sentence, the logical form of the argument is dishonest:  one is really in some way talking about the set of referents of {\bf le mrenu}, not the individual men.  A dictionary entry like {\bf clukue} as in {\bf Leu mrenu ga clukue} cannot have this behavior:  the logic engine cannot be expected to divine different logical behavior for each predicate.  So a set or partitioned mass subject must be used with a dictionary word with reciprocal meaning.  In this particular case I think a partitioned mass works well.

The second order predicate variables, {\bf bua} and its kin, are problem children.  The difficulty is that we have no mechanism to universally quantify over them (I could add a word but I'm not convinced that cmapua space should be reserved for this).  Either {\bf ra bua} or {\bf su bua} in a prenex string simply introduce a reference to {\bf bua}. which is
existentially quantified.  They both have side effects which one must bear in mind:  {\bf su bua} for example makes the sentence false if there is nothing which has the property {\bf bua}.  One can say things like {\bf Su bua goi ra bei mrenu}, in which {\bf bei} is bound by {\bf su bua}:  this says, for some property B, for all b such that B(b), b is a man.
Universal quantification over predicates can for the moment only be achieved by negation of existentially quantified assertions:  {\bf No su bua goi da bua, onoi de bua} says that da and de are the same (have exactly the same properties) if you uncoil it.  {\bf No ra bua goi da bua, onoi de bua} says essentially the same thing!

	The order of expansion is the same:  {\bf Ra mrenu ga bua} says...each man has his qualities (not necessarily the same ones for each man).  {\bf Ga bua, ga ra mrenu} says, there is a quality which inheres in every man.  {\bf Ga bua nu ganble, ga ra pernu} says that there is a way in which every person is admirable.  Yes, we can say things
with {\bf bua}.

I note the formal possibility of reserving all numerically suffixed forms of {\bf bua} and its kin as second order predicate variables, if we want them, and of introducing a special operator which implements universal quantification over predicates, if it is wanted (another approach would be to have separate predicate variables which are universally quantified).

Thinking aloud here about transformations of adverbially modified predicates (``metaphors").  It is clear from L1 that {\tt preda ca prede predi} means ``{\bf preda predi} or {\bf prede predi}" and similarly for {\bf ce} and other binary connectives, and {\tt preda prede ce predi} means {\bf preda prede} and {\bf preda predi} and similarly for other binary connectives:  or such is the intention.  But what does {\bf no} do?   I have the perhaps weird suggestion that {\bf no slano dzoru} officially expands to ``not {\bf slano dzoru} and {\bf slano canoi slano dzoru}":  it isn't slow walking, and it is the sort of walking that the qualifier ``slow" indicates we are talking about.  An anti-social butterfly is then not a social butterfly and is the sort of butterfly of which we say social or antisocial.  Social butterflies are not actually butterflies.  I don't know of course what an antisocial butterfly is, but we do stipulate as a logical requirement that {\bf no preda prede} and {\bf preda prede} are incompatible.  I suspect though that the antisocial butterfly, like the social butterfly, does not have brightly colored wings.
{\bf preda no prede} is harder to analyze.  I hazard  ``not {\bf preda prede} and {\bf preda prede canoi prede} again, where the second term seems to say that {\tt preda} is being applied in the sense in which it is applied to {\bf prede}'s'  {\bf redro no hasfa} is something which is not a red house and which moreover is red in the way way apply red to houses:  it might be a red trailer).  This might work sensibly.  If modification pathways are sensible (if we can assure the reader that {\bf raba redro canoi redro hasfa, o  hasfa}) then we can draw more conclusions about {\bf su no redro hasfa} (that it is a house!).  With these provisos we should be able to make sense of all logical operations on adverbially modified predicates, though when the modification pathways are perverse, the results may be perverse or even funny.  The fact that Loglan leads us to contemplate the antisocial butterfly I think is rather wonderful.  Perhaps it is someone who hovers around the edges of social events, magnificently dressed, and doesn't talk to anyone.  Now all of this is summed up by requiring that the binary connectives
behave as above, and that {\bf preda prede} is inconsistent wiht {\bf no preda prede} and with {\bf prede no prede}:  the expansion rules I suggest simply follow from these conditions.  The one other logical feature which {\bf preda prede} inherits from its components is that it has the same arguments as {\bf prede} (though they may not have the same meanings).
