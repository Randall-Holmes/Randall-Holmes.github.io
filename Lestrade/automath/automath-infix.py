...
# Pi Day flagship version, optimized to run fast.  Short display on by default.
# Toggle installed to turn on use of standardized keys in equation caching,
# but it is off by default.

# projects:  implement infix notation.  Current idea:  leading parentheses
# indicate prefixed part of argument list.  Add identifiers of the form
# '...' (as in Automath historical texts): allow an identifier before
# a quoted construction to be read as a one item argument list and
# a single term after it to be read as a one item argument list.  Quoted
# identifiers are always constructions.  This should give a quite sensible
# infix notation style.
...
# 3/17 infix declaration and rough infix display implemented.  Argument list
# shortening for infix terms not supported.  In back of mind, what about dangerous
# argument list shortening when names of variables agree with names of variables
# in the current context, not the context of original declaration?  Parsing of
# infix terms to come next.  Infix parsing is also installed.  It is all
# quite kludgy -- both sides of an infix are argument lists and so must
...
def VarList(L):
    ...
    if isAutomathApp(L):  return VarList(L[1]) + VarList(L[2])
    if isAutomathAbstract(L):  return [L[1]]+VarList(L[2])+VarList(L[3])
...
def AutomathDisplay(L):
    ...
    if isAutomathConstruct(L) and not(L[1] in Context) and Getconstruction((L[1]),L[3])[3]==Getconstruction(L[1],-1)[3]:
              if len(L[2])>=2 and L[1][0]=="'":  return "("+AutomathDisplay(L[2][0])+")"+L[1]+"("+AutomathListDisplay(L[2][1:])
              
              return L[1]+'('+AutomathListDisplay(L[2])
    if isAutomathConstruct(L):
              if len(L[2])>=2 and L[1][0]=="'":  return "("+AutomathDisplay(L[2][0])+")"+L[1]+str(L[3])+"("+AutomathListDisplay(L[2][1:])
...
def AutomathShortDisplay(L):
    ...
    if isAutomathConstruct(L) and not(L[1] in Context) and Getconstruction((L[1]),L[3])[3]==Getconstruction(L[1],-1)[3]:
              if len(L[2])>=2 and L[1][0]=="'":  return "("+AutomathShortDisplay(L[2][0])+")"+L[1]+"("+AutomathShortListDisplay(L[2][1:],addones(Context[1:]),False)
              LL=AutomathShortListDisplay(L[2],addones(Context),True)
              if LL=='': return L[1]
              return L[1]+'('+LL
    if isAutomathConstruct(L):
              if len(L[2])>=2 and L[1][0]=="'":  return "("+AutomathShortDisplay(L[2][0])+")"+L[1]+str(L[3])+"("+AutomathShortListDisplay(L[2][1:],addones(Context[1:]),False)
              LL=AutomathShortListDisplay(L[2],addones(Context),True)
              if LL=='': return L[1]
              return L[1]+'.'+str(L[3])+'('+LL
...
def AutomathShortListDisplay(L,C,b):
    ...
    if (b==True and not(C==[])) and L[0]==C[0] and Typeof(L[0])==Typeof(C[0]): return AutomathShortListDisplay(L[1:],C[1:],True)
    if len(L) == 1:
...
# there is now kludgy infix parsing with no argument list shortening
...
def GettermPair(s):
    ...
    if not(S=='ERROR') and (R=='' or not(R[0]=='(')):
    ...
    if s[0]=='(':
        ...
        if R2[0:1]=='.':
            ...
            return[[2,A2,a1+[A3[0]],Getconstruction(A2,-1)[3]],A3[1]]
        if not(R2[0:1]=='('):  return [[0,'bad infix term (3)'],s]
        A3=GettermlistPair(R2[1:])
        if A3[0]=='ERROR': return [[0,'bad infix term'],s]
        a3=A3[0]
        R3=A3[1]
        return [[2,A2,a1+a3,Getconstruction(A2,-1)[3]],R3]
...
def Getconstruction(s,n):
    ...
    if isident(s) and n==-1:
        original=True
        return 'ERROR'
    if not(isident(s)) and n==-1 and original==True:
        original=False
        S=Getident0(s,1)
        R=s[len(S)+1:-1]
...
# save the context for error recovery
...
# persists.
    ...
    if s=='00' or s in Context or '"' in s or '#' in s or '.' in s:
        ...
        return 'ERROR 1'
    #if s in Constructors:
        #errormessage(' previously defined constructor overwritten\n% '+ShowConstruction(s,-1))
    if t=='00':L=[[],[]]
    if samecontext:
        L=[Context,TypedContext]
...
def FixArguments(s,L,n):
     ...
     if LL2 == TypedContext[0:len(LL)-len(L)]:
