...
# April 10 -- added underscores as characters in identifiers, to support
# a style in the yellow book.

# April 5 -- edited remarks throughout the file, trying to remove
# anachronisms.

# April 2 -- imposing insane subtyping on propositional types, so that
# quantification over any type (propositional or object) is supported.
# This is justifiable, though it may imply propositional irrelevance.
# (I'm not convinced that it does, but it might;  the models that
# show it works do have propositional irrelevance).

# This allows equality to be defined (not needed as a primitive).  This
# is useful if rewriting is to be implemented.  See defeq.aut1.  It does
# also immediately mean that the False is definable (as [p:prop]p)

# Insane typing on object types is much more obviously dangerous, though
# interesting.

# Provided a toggle to turn off the liberalized universal quantification,
# restoring the normal AUT-QE situation
# and also a toggle which turns off all subtyping, giving
# AUT-QE-NT.

# I'm designating the default dialect of MANIAC as AUT-QE2, so called
# because it allows general abstraction over things of degree 2 (sorts).
# the admission of all universal quantifiers as definable seems to
# fit the spirit of AUT-QE thus expanded.

# By turning off construction types one just gets AUT-QE.
...
# toggle admitting insanely dangerous impredicative subtyping

insanetypes=False

# toggle admitting all universal quantifiers (on by default)

alluniversals=True

# toggle which turns off all subtyping of metasorts
...
def ToggleAllUniversals():
    global alluniversals
    alluniversals=not alluniversals
    if alluniversals:  print("All universal quantifiers on")
    if not alluniversals:  print("Restricted universal quantifiers on")

def ToggleNoSubtyping():
    global nosubtyping
    nosubtyping=not nosubtyping
    if nosubtyping:  print("Subtyping is turned off")
    if not nosubtyping:  print("Subtyping is turned on")
...
def isident(s):
    ...
    return (len(s)>0 and (isdigit(s[0]) or isupper(s[0]) or islower(s[0]) or s[0]=='_' or s[0]=='#') and ((len(s)==1) or (len(s)==2 and (s[1]=='$' or s[1]=='!')) or isident(s[1:])))
...
# to constructions (represented as lambda terms with types which may be
# impossible in the original Automath type system, as input types may be metasorts).
...
# evaluation = definition expansion, under some settings.
...
# the tweak, which is on by default now, allows MANIAC to look for declarations of identifiers
# in the most recently closed paragraph, as a last resort.
...
# converse-applied to a function (not necessarily legal in classic Automath) representing
# the construction.
...
# use declaration of a construction to build a (likely illegal in classic Automath) function
# representing it.
...
# argument before reducing.  The toggle is historical,
...
def EqualTerms(L,M):
    ...
    if (not nosubtyping) and istypetype(M) and istypetype(L) and (not(isAutomathAbstract(L)) and isAutomathAbstract(M)) and ((alluniversals and L==[4,'prop']) or insanetypes or istypetype(Typeof(M[2]))):
        V=FreshIdent(M[1])
        Context=Context+[V]
...
# check that a term is a metasort
