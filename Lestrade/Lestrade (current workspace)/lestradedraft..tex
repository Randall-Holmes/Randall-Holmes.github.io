...
The rewrite and deferred type labels have been eliminated.

We outline the scheme for computation which we now have in mind.

A new {\tt program} command to be introduced.  The format is {\tt program f x1 ... xn, t1:t2} where the {\tt xi}s are parameters in move 1 (this command can only be issued in move 1) and {\tt t1} and {\tt t2} are terms.
The intention is that {\tt t1} and {\tt t2} are equal for all values of the parameters appearing in them, and that {\tt f}
is a final projection operator, so execution behavior rewriting   {\tt f x1 ... xn, t1} to {\tt t2} is sound.

Every program command is in effect introducing an axiom.  There is no version analogous to the {\tt rewrited}
of Lestrade 1.0 (justified by a theorem).  We can nonetheless use theorems to justify rewrites, by declaring a program that rewrites x to y with a proof of x=y as an argument.

The effect of the command is to declare as an identifier of type {\tt [x1 ... xn tau => tau]} where {\tt tau} is the common type of {\tt t1} and {\tt t2} [so it is a prerequisite for success of the command that {\tt t1} and {\tt t2} have the same type].

Economical use of patterns is intended to be handled by the ability to make any or all of the $x_i$'s implicit if they can be deduced from {\bf t1}.
...
else "("^(if t <> Unknown then "{def} " else "")^
...
We intend to add a command which causes execution behavior (rewriting) but the details are not clear yet.
