...
\date{1/27/2026:  a new draft aiming for a new approach to rewriting and other issues.  This is a PolyML version.}
...
\begin{description}
\item[1/27/2026:]  Aiming for a new rewrite system more like the one in Lestrade 1.0.  Starting by stripping out the rewrite type.   A description of the intended system will appear here or in the main text after I remove the rewrite type.

Also, read the document.

It would be handy to introduce an order which can be placed in executable Lestrade text embedded in a .tex file to force a page break when it is run.  Inserting text comments might also be convenient.

\item[9/9/2024:]  Contemplating the ability to declare defined notions that do not expand.  Such defined notions probably tagged by some feature of the identifier.
The idea is that definitional expansion does nothing to them, and their definitions are copied into the last move when the next move is closed (this might cause expansion though, it requires care).   Because of these features, there is no danger of masking their names.

\item[2/28/2024:]  I believe that I should implement computation in this prover.

The sketch I have in mind is an implementation of something like the system of the first version,
where a function can have one or more rewrite rules (of quite sterotyped form) bound to it to be applied
when that function appears in applied position.  This has the dependency issues:  rewrite rules would need
to be declared only at the top level.

I'm getting a mental glimmering of the approach I seem to have had in mind when I was working on this
code, and I'll try sketching it in this note.  The idea is that a rewrite rule to be used in computation of a function
is supplied as an argument to that function.   This means that variables of rewrite type have to be supplied
for these declarations...but they can be instantiated by top level rewrite rules in actual applications.

The two approaches could be combined.  Rewrite rules supplied as types of arguments would have to be of the stereotyped
form, and would be attached to leading functions on their left sides and so used in a much more efficient way --
but could then be used in any move and dependencies would be properly tracked.   This could give a lot of power
and a lot of fine control without an excessive amount of extra matching.

This is quite exciting.  And it would pretty clearly support most features of the Watson rewrite system.

One point is that a rewrite would not be applied unless an actual argument of the required rewrite type was given.
An uninstantiated rewrite argument prevents expansion of the definition (which would make the parameter disappear,
corrupting dependencies).  This really requires very careful thought.

I note to myself that I also have implementation projects.  I want to do something with HOTT better than my original presentation (with my new correct understanding of univalence).  I also want to take a stab at the untyped type theory in Lestrade.  This seems somehow appropriate.
...
Matching of a pattern to a term will exclude matching of its non-at
omic subterms to terms that match patterns:  this cleverly enforces Curry-Howard.
...
could have happened with strong expansion, too;  my good fortune.
...
This document is intended to serve as a specification of the Lestrade dependent type system and to contain code for the implementation (the Lestrade Type Inspector)  in parallel with the specification.    This version runs in PolyML.  There are code modifications indicated in the preamble which may cause it to run correctly in other ML versions.
...
This version has met a benchmark:  it runs the entire Zermelo implementation with minor modifications due to the quite minor changes in the language between Lestrade 1.0 and Lestrade 2.0.
...
all variables taken from the next move in a rewrite object are actually bound.
...
(*   DELETE  Deferred (* the unknown output value 
...
(* DELETE   Rewrite of (Object*Object) 
...
only the number of explicit arguments counts (the arity in the following discussion).    The display function uses the format of a declared operator to determine
...
(*  DELETE typedeplist(Rewrite(t,u)) =
...
(* DELETE typeerrorsfound (Rewrite(t,u)) = 
...
(* DELETE typeoccurrences s m n (Rewrite(t,u)) =
...
(* DELETE maxdepthtype (Rewrite(t,u)) =
...
(* DELETE displaytype (Rewrite(t,u)) = 
"{"^(displayobject t)^"-->"
^(displayobject u)^"}" | *)
...
else "("^(if t <> Unknown (* DELETE andalso t <> Deferred *) then "{def} " else "")^
...
(* DELETE Substype0 depth s m n b T (Rewrite(TT,UU)) =
...
(* DELETE equalobjectsorts (Rewrite(t1,u1)) (Rewrite(t2,u2))  (* is this actually correct?  might literal identity be required? *)
...
(* DELETE unsubstype S x (Rewrite(t,u)) =
...
(* DELETE matchsubstype depth match (Rewrite(t,u)) =
...
(* DELETE matchtypest v (Rewrite(t1,u1)) (Rewrite(t2,u2)) = (* same query -- might matching of rewrite types be very literal? *)
...
(* DELETE objectsortcheck (Rewrite(t,u)) = equalfunctions(objectsort t)(objectsort u) |  *)
...
(* DELETE typefixtype (Rewrite(t,u)) = 
