...
(* this version is marked up by Randall Holmes as he learns *)
...
(* my example for Morgan, proved backward then forward *)

Theorem  Morgan_Example: (forall A B C:Prop, (A->B->C)->(B->A->C)).
Proof.
intros A B C.
intros the_implication.
intros suppose_b.
intros suppose_a.
refine(the_implication _ _).
exact suppose_a.
exact suppose_b.

Qed.

Theorem  Morgan_Example_2: (forall A B C:Prop, (A->B->C)->(B->A->C)).
Proof.
intros A B C.
intros the_implication.
intros suppose_b suppose_a.
pose(line1:=the_implication suppose_a).
pose(line2:=line1 suppose_b).
exact line2.
