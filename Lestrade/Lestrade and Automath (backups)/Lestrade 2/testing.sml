...
\date{5/28/2021  installation of rewrites in progress \\
...
\item[5/28/2021:]  Starting installation of the actual rewriting mechanism.

Created boundvarlist function which generates list of variables in a lambda term.

Put the matching functions in the recursive package with typing and equality.

\item[5/26/2021:]  Now I have to think about how to install actual rewriting now that I have the type.

Beta reduction and type reduction need to load rewrite rules into a list of rewrite rules (probably a stack of lists of rewrite rules) and then apply rewrites to definition output
and use rewrites to assist identification of types, much as definition expansion is used.

an essential part of this is application of a single rewrite.

Anothe part is recognition of arguments of rewrite type and loading them into the top rewrite list, and then management of the stack of rewrite lists.
...
and  referentof2(s,m,n,b,T) = if isobjecttype T 
then Lambda(nil,Oatom(s,m,n,b),Tdummy)
else Fatom(s,m,n,b)

and obvarpackage(s,m,n,b) = Lambda(nil,Oatom(s,m,n,b),Tdummy)

and boundvarlist (Lambda(((s,(i,m,n,b,T))::L),O,TT)) =

   (referentof2(s,m,n,b,T)::(boundvarlist(Lambda(L,O,TT)))) |

boundvarlist x = nil

(* just above, list of bound variables in a 
lambda-term, for rewrite matching *)
...
and objectsorttest s = objectsort(getobject(Tokenize s))

and functionsorttest s = functionsort(getfunction(Tokenize s))
...
and varsof s =  let val SS = absolutesortof s (length(!CONTEXT)-1) in 
...
and isvariable (Lambda(nil,Oatom(s,m,0,b),Tdummy)) = m=length(!CONTEXT)-1 |

isvariable (Fatom(s,m,0,b)) = m=length(!CONTEXT)-1 |

isvariable x = false

and addmatch key value [((""),Ferror)] = [((""),Ferror)] |
...
and mergematch  [((""),Ferror)] L =  [((""),Ferror)] |
...
and showmatches0 nil = " is localmatches" |

showmatches0 ((s,t)::L) = s^(":  ")
^(displayfunction t)^"; "^(showmatches0 L)

and showmatches() = showmatches0 (!LOCALMATCHES)

and addtomatches key value match = 
...
and matchesfail() = (LOCALMATCHES:=[("",Ferror)];[("",Ferror)])

(* substitutions using a match *)

and matchsubstype depth match (THAT p) = 
...
and matchallsubs t = matchsubsfunction 0 (!LOCALMATCHES) t
...
(* matching function with a list of source variables *)

fun matchtypesl L (THAT p) (THAT P) = matchobjectsl L p P |

matchtypesl L  (IN p) (IN P) = matchobjectsl L p P |

matchtypesl L Terror Terror = matchesfail() |

matchtypesl L (Rewrite(t1,u1)) (Rewrite(t2,u2)) =

let val MMM = matchobjectsl L t1 t2 in

let val M=mergematch (MMM) (matchtypesl L u1 u2)
...
matchtypesl L x y = if x=y then nil else  matchesfail()



and matchobjectsl L (Oatom(s,m,n,b)) T = 

if find0 (obvarpackage(s,m,n,b)) L then

if equalobjects (Oatom(s,m,n,b)) T then nil
...
and matchtypest v (THAT p) (THAT P) = 
