...
# often.  This was already true for display, so it had better be true for parsing!
...
def AutomathShortDisplay(L):
    ...
    if isAutomathConstruct(L) and not(L[1] in Context) and Getconstruction((L[1]),L[3])[3]==Getconstruction(L[1],-1)[3]:
              ...
              if len(LL)==2 and L[1][0]=="'":  return unless(isAutomathVar(LL[0]),"(")+AutomathShortDisplay(LL[0])+unless(isAutomathVar(LL[0]),")")+L[1]+AutomathShortDisplay(LL[1])
    ...
    if isAutomathConstruct(L):
              ...
              LL=AutomathShortList(L[2],addones(Context),True)
              if len(LL)==2 and L[1][0]=="'":  return unless(isAutomathVar(LL[0]),"(")+AutomathShortDisplay(LL[0])+unless(isAutomathVar(LL[0]),")")+L[1]+str(L[3])+AutomathShortDisplay(LL[1])
...
def AutomathShortList(L,C,b):
    ...
    if (b==True and not(C==[])) and L[0]==C[0] and renameallx(Typeof(L[0]))==renameallx(Typeof(C[0])): return AutomathShortList(L[1:],C[1:],True)
    if len(L) == 1:
        return [L[0]]
    if C==[]:Cshorter=[]
    if not C== []: Cshorter=C[1:]
