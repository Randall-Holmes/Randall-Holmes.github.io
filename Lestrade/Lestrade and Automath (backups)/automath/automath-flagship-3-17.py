...
def AutomathDisplay(L):
    ...
    if isAutomathConstruct(L) and not(L[1] in Context) and Getconstruction((L[1]),L[3])[3]==Getconstruction(L[1],-1)[3]:
              if len(L[2])==2 and L[1][0]=="'":  return unless(isAutomathVar(L[2][0]),"(")+AutomathDisplay(L[2][0])+unless(isAutomathVar(L[2][0]),")")+L[1]+AutomathDisplay(L[2][1])
    ...
    if isAutomathConstruct(L):
              if len(L[2])==2 and L[1][0]=="'":  return unless(isAutomathVar(L[2][0]),"(")+AutomathDisplay(L[2][0])+unless(isAutomathVar(L[2][0]),")")+L[1]+str(L[3])+AutomathDisplay(L[2][1])
...
def AutomathShortDisplay(L):
    ...
    if isAutomathConstruct(L) and not(L[1] in Context) and Getconstruction((L[1]),L[3])[3]==Getconstruction(L[1],-1)[3]:
              if len(L[2])==2 and L[1][0]=="'":  return unless(isAutomathVar(L[2][0]),"(")+AutomathShortDisplay(L[2][0])+unless(isAutomathVar(L[2][0]),")")+L[1]+AutomathShortDisplay(L[2][1])
              if len(L[2])>2 and L[1][0]=="'":  return unless(isAutomathVar(L[2][0]),"(")+AutomathShortDisplay(L[2][0])+unless(isAutomathVar(L[2][0]),")")+L[1]+"("+AutomathShortListDisplay(L[2][1:],addones(Context[1:]),False)
              LL=AutomathShortListDisplay(L[2],addones(Context),True)
              if LL=='': return L[1]
              return L[1]+'('+LL
    if isAutomathConstruct(L):
              if len(L[2])==2 and L[1][0]=="'":  return "("+AutomathShortDisplay(L[2][0])+")"+L[1]+str(L[3])+AutomathShortDisplay(L[2][1])
...
def GettermPair(s):
    ...
    if not(S=='ERROR') and (R=='' or (not(R[0]=='(') and not(R[0]=="'"))):
    ...
    if not(S=='ERROR') and R[0]=="'" and N==-1:
        return GettermPair('('+S+')'+R)   
    if not(S=='ERROR') and R[0]=="'":
