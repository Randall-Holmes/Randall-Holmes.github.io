...
define Iffrefl p: Dediff [pp => pp] \
   [pp => pp] \
   



>> Iffrefl: [(p_1:prop) => (Dediff([(pp_2:that
>>         p_1) => (pp_2:that p_1)]
...
>> Counterexample0: [(pred_1:[(x_2:obj) => (---:
>>         prop)]),
>>      (negunivev_1:that ~(Forall(pred_1))) =>
>>      (Counterexample(negunivev_1):that Exists([(z_3:
>>         obj) => (~(pred_1(z_3)):prop)]))
>>      ]
>>   {move 0}
...
>> uniqueev: that One(pred) {move 1}



declare impliesev that Forall [x=> (pred \
      x) == pred2 x] \
   



>> impliesev: that Forall([(x_1:obj) => ((pred(x_1)
>>      == pred2(x_1)):prop)])
>>   {move 1}
